...
exports.deleteFirm = (req, res) => {
    ...
    try {
        ...
        const firmUsers = db.prepare('SELECT COUNT(*) as count FROM users WHERE firm_id = ?').get(id);
        ...
        const firmStocks = db.prepare('SELECT COUNT(*) as count FROM stocks WHERE firm_id = ?').get(id);
        const firmBills = db.prepare('SELECT COUNT(*) as count FROM bills WHERE firm_id = ?').get(id);
        const firmParties = db.prepare('SELECT COUNT(*) as count FROM parties WHERE firm_id = ?').get(id);
