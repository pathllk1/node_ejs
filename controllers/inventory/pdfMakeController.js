...
exports.getBillPdf = async (req, res) => {
    try {
        ...
        const items = db.prepare('SELECT *, item_narration FROM stock_reg WHERE bill_id = ? ORDER BY created_at').all(id);
        ...
        if (req.user && req.user.firm_id) {
            const firmGstSetting = db.prepare('SELECT setting_value FROM firm_settings WHERE firm_id = ? AND setting_key = ?').get(req.user.firm_id, 'gst_enabled');
            if (firmGstSetting) gstEnabled = JSON.parse(firmGstSetting.setting_value);
            else {
                const globalGst = db.prepare('SELECT setting_value FROM settings WHERE setting_key = ?').get('gst_enabled');
        ...
        if (req.user && req.user.firm_id) {
            const firm = db.prepare('SELECT name, address, contact_info FROM firms WHERE id = ?').get(req.user.firm_id);
            if (firm) {
        ...
        const docDefinition = {
            ...
            content: [
                        ...
                        },
                        {
                            ...
                            table: {
                                ...
                                body: [
                                    ...
                                    [{ text: 'Date:', style: 'label' }, { text: bill.bdate || '', style: 'value' }],
                ...
                // Items Table
                {
                    table: {
                        headerRows: 1,
                        widths: [15, 135, 50, 30, 25, 40, 30, 30, 60],
                        body: [
                            [
