...
exports.signup = async (req, res) => {
    try {
        ...
        const checkUserResult = await turso.execute({
        ...
        await turso.execute({
            sql: `
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `,
            args: [fullname, username, email, hashedPassword, now, now, null, null]
        });
...
exports.login = async (req, res) => {
    try {
        ...
        // 1. Find User
        const findUserResult = await turso.execute({
            sql: 'SELECT *, firm_id FROM users WHERE email = ?',
            args: [email]
        });
...
exports.getUserProfile = async (req, res) => {
    try {
        ...
        const userResult = await turso.execute({
            sql: 'SELECT id, fullname, username, email, created_at, firm_id FROM users WHERE id = ?',
            args: [userId]
        });
