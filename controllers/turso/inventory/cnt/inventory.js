    ...
    try {
        ...
        const billsQuery = await turso.execute({
            sql: `
                ...
                WHERE b.firm_id = ? AND b.btype = 'CREDIT_NOTE'
...
exports.getNextBillNumber = async (req, res) => {
    try {
        ...
        console.error('[CNT_GET_NEXT_BILL_INFO] Error:', error.message);
        res.status(500).json({ error: `Failed to get bill number info: ${error.message}` });
    }
};

exports.getAllBills = exports.getAllCreditNotes;
    ...
    try {
        ...
        const billQuery = await turso.execute({
            sql: `
                ...
                WHERE b.id = ? AND b.firm_id = ? AND b.btype = 'CREDIT_NOTE'
            `,
    ...
    try {
        const billInsert = await turso.execute({
            ...
            args: [
                ...
                gtot, ntot, rof, 'CREDIT_NOTE', actorUsername, party.firm,
        ...
        await turso.execute({
            ...
            args: [
                ...
                0, ntot, `Credit Note No: ${meta.billNo}`, ledgerBase.bill_id, party.id || null,
        ...
        if (cgst > 0) {
            await turso.execute({
                ...
                args: [
                    ...
                    cgst, 0, `CGST reversal on Credit Note: ${meta.billNo}`, ledgerBase.bill_id, null,
        ...
        if (sgst > 0) {
            await turso.execute({
                ...
                args: [
                    ...
                    sgst, 0, `SGST reversal on Credit Note: ${meta.billNo}`, ledgerBase.bill_id, null,
        ...
        if (igst > 0) {
            await turso.execute({
                ...
                args: [
                    ...
                    igst, 0, `IGST reversal on Credit Note: ${meta.billNo}`, ledgerBase.bill_id, null,
        ...
        if (Math.abs(parseFloat(rof)) > 0) {
            ...
            await turso.execute({
                ...
                args: [
                    ...
                    rofVal > 0 ? rofVal : 0, rofVal < 0 ? Math.abs(rofVal) : 0, `Round off reversal on Credit Note: ${meta.billNo}`,
                ...
                if (chargeAmount > 0) {
                    await turso.execute({
                        ...
                        args: [
                            ...
                            chargeAmount, 0, `${charge.name || charge.type || 'Other Charges'} reversal on Credit Note: ${meta.billNo}`,
                            ledgerBase.bill_id, null,
                            null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                            ledgerBase.created_at, ledgerBase.updated_at
                        ]
        ...
        await turso.execute({
            ...
            args: [
                ...
                taxableItemsTotal, 0, `Sales return on Credit Note: ${meta.billNo}`,
                ledgerBase.bill_id, null,
                null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                ledgerBase.created_at, ledgerBase.updated_at
            ]
...
exports.updateBill = async (req, res) => {
    ...
    const existingBillQuery = await turso.execute({
        ...
        args: [id, req.user.firm_id, 'CREDIT_NOTE']
    });
    ...
    try {
        // A) Restore stock to pre-credit-note state (subtract previously added qty)
        ...
        // D) Apply new stock changes (add qty)
        for (const item of cart) {
            const lineTotal = item.qty * item.rate * (1 - (item.disc || 0) / 100);
        ...
        await turso.execute({
            ...
            args: [
                ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, party.firm, 'DEBTOR',
                0, ntot, `Credit Note No: ${meta.billNo} (Updated)`, ledgerBase.bill_id, party.id || null,
                null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                ledgerBase.created_at, ledgerBase.updated_at
            ]
        });
        ...
        if (cgst > 0) {
            await turso.execute({
                ...
                args: [
                    ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, 'CGST', 'TAX',
                    cgst, 0, `CGST reversal on Credit Note: ${meta.billNo} (Updated)`, ledgerBase.bill_id, null,
                    'CGST', null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                    ledgerBase.created_at, ledgerBase.updated_at
                ]
            });
        ...
        if (sgst > 0) {
            await turso.execute({
                ...
                args: [
                    ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, 'SGST', 'TAX',
                    sgst, 0, `SGST reversal on Credit Note: ${meta.billNo} (Updated)`, ledgerBase.bill_id, null,
                    'SGST', null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                    ledgerBase.created_at, ledgerBase.updated_at
                ]
            });
        ...
        if (igst > 0) {
            await turso.execute({
                ...
                args: [
                    ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, 'IGST', 'TAX',
                    igst, 0, `IGST reversal on Credit Note: ${meta.billNo} (Updated)`, ledgerBase.bill_id, null,
                    'IGST', null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                    ledgerBase.created_at, ledgerBase.updated_at
                ]
            });
        ...
        if (Math.abs(parseFloat(rof)) > 0) {
            ...
            await turso.execute({
                ...
                args: [
                    ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, 'Round Off', 'INDIRECT EXPENSE',
                    rofVal > 0 ? rofVal : 0, rofVal < 0 ? Math.abs(rofVal) : 0, `Round off reversal on Credit Note: ${meta.billNo} (Updated)`,
                    ledgerBase.bill_id, null,
                    null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                    ledgerBase.created_at, ledgerBase.updated_at
                ]
                ...
                if (chargeAmount > 0) {
                    await turso.execute({
                        ...
                        args: [
                            ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no,
                            charge.name || charge.type || 'Other Charges', 'INCOME',
                            chargeAmount, 0, `${charge.name || charge.type || 'Other Charges'} reversal on Credit Note: ${meta.billNo} (Updated)`,
                            ledgerBase.bill_id, null,
                            null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                            ledgerBase.created_at, ledgerBase.updated_at
                        ]
        ...
        await turso.execute({
            ...
            args: [
                ledgerBase.voucher_id, ledgerBase.voucher_type, ledgerBase.voucher_no, 'Sales', 'INCOME',
                taxableItemsTotal, 0, `Sales return on Credit Note: ${meta.billNo} (Updated)`, ledgerBase.bill_id, null,
                null, null, ledgerBase.transaction_date, ledgerBase.created_by, ledgerBase.firm_id,
                ledgerBase.created_at, ledgerBase.updated_at
            ]
        });
