/**
 * Migration: Make amount nullable in vouchers table to support journal entries
 * Journal entries don't have a single amount value since they can have multiple debit/credit lines
 */

const turso = require('../config/turso');

const makeAmountNullableInVouchers = async () => {
    console.log('🔨 Making amount nullable in vouchers table...');
    ...
    try {
        ...
        await turso.execute(`
            CREATE TABLE vouchers_new (
                ...
                party_id INTEGER, -- Now nullable (from previous migration)
                amount REAL, -- Made nullable (removed NOT NULL) to support journal entries
        ...
        console.log('✅ vouchers table updated successfully - amount is now nullable');
        return true;
    } catch (error) {
...
const runMigration = async () => {
    console.log('🚀 Starting amount nullable migration...\n');
    
    try {
        console.log('Step 1: Making amount nullable in vouchers table');
        console.log('━'.repeat(50));
        await makeAmountNullableInVouchers();
        console.log('');
        
        console.log('✅ Amount nullable migration completed successfully!');
        console.log('━'.repeat(50));
        console.log('The vouchers table now allows NULL amount for journal entries');
        
    } catch (error) {
        console.error('\n❌ Amount nullable migration failed:');
        console.error(error);
        process.exit(1);
    }
};
