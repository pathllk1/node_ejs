document.addEventListener('DOMContentLoaded', () => {
    ...
    // [NEW] Variable to store the timer ID
    ...
    analyzeBtn.addEventListener('click', async () => {
        ...
        // 1. Reset UI State
        setLoading(true);
        hideResult(); // This starts the timer
    ...
    function showResult(data) {
        // [NEW] CRITICAL FIX: Cancel the pending hide timer!
        if (hideTimeout) {
        ...
        // Populate text
        sentimentResult.textContent = data.analysis_result;
        confidenceResult.textContent = (data.confidence_score * 100).toFixed(1) + '%';
        scoreBadge.textContent = data.analysis_result;
    ...
    function hideResult() {
        // [NEW] Clear any existing timer first
        if (hideTimeout) clearTimeout(hideTimeout);

        resultCard.classList.add('opacity-0', 'translate-y-4');
        
        // [NEW] Save the timer ID to the variable
