(function cntInitPurchaseSystem() {
    console.log('PRS: Initializing Professional Purchase System...');
    const container = document.getElementById('credit-note');
    if (!container) return;

    const prsGST_API_CONFIG = {
    ...
    async function cntPrsFetchCurrentUserFirmName() {
        try {
            const response = await window.api.get('/inventory/cnt/api/current-user-firm-name');
    ...
    cntPrsFetchCurrentUserFirmName();
    ...
    function cntPrsRemoveOtherCharge(index) {
        state.otherCharges.splice(index, 1);
        cntPrsRefreshTable();
    }
    
    function cntPrsUpdateOtherCharge(index, charge) {
        // Calculate GST amount only if GST is enabled
        if (state.gstEnabled !== false) {  // Default to enabled if not set
    ...
    function cntPrsGetTotalOtherCharges() {
    ...
    // --- DATA FETCHING ---
    async function cntPrsFetchData() {
        try {
            // 1. Fetch Stocks
            const res = await window.api.get('/inventory/cnt/api/stocks');
            ...
            // 2. Fetch Parties
            try {
                const partyRes = await window.api.get('/inventory/cnt/api/parties');
    ...
    function cntPrsRenderLayout() {
        ...
        <div class="h-[calc(100vh-140px)] flex flex-col bg-gray-50 text-slate-800 font-sans text-sm border border-gray-300 rounded-lg shadow-sm overflow-hidden">
            ...
            <div class="bg-white border-b border-gray-200 p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 shadow-sm z-20">
                <div class="flex flex-col sm:flex-row flex-wrap gap-2">
                    <div class="flex flex-col">
                        ...
                        <input type="text" value="${state.meta.cntBillNo}" id="cnt-prs-bill-no" class="border border-gray-300 rounded px-2 py-1 text-xs font-bold w-32 bg-white text-slate-700 focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Enter Bill No" title="Manual bill number entry">
                    </div>
                    <div class="flex flex-col">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select id="cnt-prs-billTypeSelector" class="border border-gray-300 rounded px-2 py-1 text-xs bg-white focus:ring-1 focus:ring-blue-500 outline-none text-slate-700 font-medium">
                    ...
                    <div class="flex items-center gap-2 pt-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" id="cnt-prs-reverse-charge-toggle" ${state.meta.reverseCharge ? 'checked' : ''} class="form-checkbox h-4 w-4 text-blue-600 rounded">
                ...
                <div class="flex flex-wrap gap-2">
                    <button id="cnt-prs-btn-other-charges" class="px-3 py-1.5 text-xs text-blue-600 border border-blue-200 bg-blue-50 rounded hover:bg-blue-100 transition-colors whitespace-nowrap">Other Charges</button>
                    <button id="cnt-prs-btn-reset" class="px-3 py-1.5 text-xs text-red-600 border border-red-200 bg-red-50 rounded hover:bg-red-100 transition-colors whitespace-nowrap">Reset</button>
                    <button id="cnt-prs-btn-save" class="px-4 py-1.5 bg-slate-800 text-white text-xs rounded hover:bg-slate-900 shadow font-medium flex items-center gap-2 transition-colors whitespace-nowrap">
            ...
            <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
                ...
                <div class="w-full md:w-64 bg-slate-50 border-r border-gray-200 flex flex-col overflow-y-auto z-10">
                    ...
                    <div class="p-3 border-b border-gray-200 bg-white">
                        <div class="flex justify-between items-center mb-1">
                            <label class="text-[10px] uppercase text-gray-500 font-bold tracking-wider">Bill From (Supplier)</label>
                        </div>
                        <div id="cnt-prs-party-display">
                            <div class="group bg-blue-50 p-3 rounded border border-blue-200 shadow-sm">
                                <div>
                                    ...
                                    <div class="mt-2 text-right">
                                        <button id="cnt-prs-btn-change-party" class="text-[10px] text-blue-600 hover:text-blue-800 font-bold bg-white px-2 py-1 rounded shadow-sm border border-gray-200 hover:border-blue-300 whitespace-nowrap">
                                            Change
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div class="p-3 space-y-3">
                         <div>
                            ...
                            <input type="text" id="cnt-prs-reference-no" value="${state.meta.referenceNo}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="e.g. PO-2025-001">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Vehicle No</label>
                            <input type="text" id="cnt-prs-vehicle-no" value="${state.meta.vehicleNo}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="XX-00-XX-0000">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Dispatched Through</label>
                            <input type="text" id="cnt-prs-dispatch-through" value="${state.meta.dispatchThrough}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="Courier / Transport">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Narration</label>
                            <textarea id="cnt-prs-narration" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none h-20 resize-none" placeholder="Additional notes...">${state.meta.narration}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 bg-white flex flex-col relative min-w-0">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar relative" id="cnt-prs-items-container">
                        ${cntPrsRenderItemsList()}
                    </div>

                    <div class="p-2 border-t border-dashed border-gray-200 bg-gray-50 shrink-0">
                        <button id="cnt-prs-btn-add-item" class="w-full py-2 border border-dashed border-blue-300 text-blue-600 rounded hover:bg-blue-50 text-xs font-bold transition-colors uppercase tracking-wide">
                            + Add Items (F2) | Select Supplier (F3) | Charges (F4)
                        </button>
                    </div>

                    <div class="bg-slate-50 border-t border-slate-300 p-4 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]" id="cnt-prs-totals-section">
                        ${cntPrsRenderTotals()}
                    </div>
                </div>
            </div>
        ...
        <div id="cnt-prs-modal-backdrop" class="fixed inset-0 bg-black/50 hidden z-40 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="cnt-prs-modal-content" class="bg-white rounded shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-down">
                </div>
        </div>

        <div id="cnt-prs-sub-modal-backdrop" class="fixed inset-0 bg-black/60 hidden z-50 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="cnt-prs-sub-modal-content" class="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-300 animate-scale-in">
                </div>
        </div>
        
        <!-- Other Charges Modal -->
        <div id="cnt-prs-other-charges-modal-backdrop" class="fixed inset-0 bg-black/60 hidden z-50 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="cnt-prs-other-charges-modal-content" class="bg-white rounded-lg shadow-2xl w-full max-w-3xl overflow-hidden border border-gray-300 animate-scale-in">
                <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                    <h3 class="font-bold text-sm tracking-wide">OTHER CHARGES</h3>
                    <button id="cnt-prs-close-other-charges-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
                </div>
                
                <div class="p-6">
                    <div class="flex gap-4 mb-4">
                        <div class="relative flex-1">
                            <input type="text" id="cnt-prs-charge-name" placeholder="Charge Name" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                            <div id="cnt-prs-charge-name-suggestions" class="absolute z-10 w-full min-w-[400px] bg-white border border-gray-300 rounded shadow-lg mt-1 max-h-40 overflow-y-auto hidden"></div>
                        </div>
                        <input type="text" id="cnt-prs-charge-hsn" placeholder="HSN/SAC" class="w-24 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none" title="Enter HSN for goods or SAC for services">
                        <input type="number" id="cnt-prs-charge-amount" placeholder="Amount" step="0.01" class="w-32 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                        <input type="number" id="cnt-prs-charge-gst" placeholder="GST %" step="0.01" class="w-24 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                        <select id="cnt-prs-charge-type" class="w-40 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none bg-white">
                        ...
                        <button id="cnt-prs-add-charge-btn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors whitespace-nowrap">
                            ADD
                        </button>
                    </div>
                    ...
                    <div class="mb-4 text-sm text-gray-600">
                        ...
                        <span id="cnt-prs-total-other-charges" class="font-bold text-blue-700">0.00</span>
                    </div>
                    ...
                    <div class="overflow-y-auto max-h-80">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody id="cnt-prs-other-charges-list" class="text-xs text-gray-700 divide-y divide-gray-100">
                                ${cntPrsRenderOtherChargesList()}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="p-4 border-t border-gray-200 flex justify-end gap-3">
                    <button id="cnt-prs-cancel-other-charges" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
                    <button id="cnt-prs-save-other-charges" class="px-6 py-2 bg-green-600 text-white text-sm font-bold rounded shadow hover:bg-green-700 transition-colors">DONE</button>
                </div>
            </div>
        </div>
        `;
        ...
        const partyContainer = document.getElementById('cnt-prs-party-display');
        if (partyContainer) {
            cntPrsRenderPartyCard().then(html => {
                partyContainer.innerHTML = html;
                
                // Attach event listeners for both change and select buttons
                const changeBtn = document.getElementById('cnt-prs-btn-change-party');
                if (changeBtn) changeBtn.addEventListener('click', cntPrsOpenPartyModal);
                
                const selectBtn = document.getElementById('cnt-prs-btn-select-party');
                if (selectBtn) selectBtn.addEventListener('click', cntPrsOpenPartyModal);
            });
        }
    ...
    function cntPrsRenderOtherChargesList() {
        ...
        return state.otherCharges.map((charge, index) => {
            ...
            <tr class="hover:bg-blue-50 transition-colors">
                ...
                <td class="p-3 text-right font-bold text-gray-800">${cntPrsFormatCurrency(charge.amount)}</td>
                <td class="p-3 text-right font-bold text-gray-800">${(charge.gstRate || 0)}%</td>
                <td class="p-3 text-center">
                    <button class="prs-btn-remove-charge text-red-600 hover:text-red-800 transition-colors font-bold text-lg leading-none" data-index="${index}">&times;</button>
                </td>
            </tr>
            <tr class="hover:bg-blue-50 transition-colors bg-gray-50">
                ...
                <td class="p-1 text-right text-gray-500 text-xs">${cntPrsFormatCurrency(gstAmount)}</td>
                <td class="p-1 text-right text-gray-500 text-xs font-bold">Total:</td>
                <td class="p-1 text-right text-gray-800 text-xs font-bold">${cntPrsFormatCurrency(totalAmount)}</td>
            </tr>`;
        }).join('');
    }
    
    async function cntPrsRenderPartyCard() {
        if (state.selectedParty) {
            // Fetch party balance
            let balanceInfo = null;
            try {
                const balanceResponse = await window.api.get(`/inventory/prs/api/parties/${state.selectedParty.id}/balance`);
                ...
                <div class="group bg-blue-50 p-3 rounded border border-blue-200 shadow-sm">
                    <div>
                        <div class="flex justify-between items-start">
                            ...
                            <button id="cnt-prs-btn-change-party" class="text-[10px] text-blue-600 hover:text-blue-800 font-bold bg-white p-1.5 rounded shadow-sm border border-gray-200 hover:border-blue-300 whitespace-nowrap ml-2" title="Change Supplier">
        ...
        return `
            <button id="cnt-prs-btn-select-party" class="w-full py-6 border-2 border-dashed border-gray-300 rounded-lg text-gray-400 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-all flex flex-col items-center justify-center gap-2 group">
                <span class="text-2xl group-hover:scale-110 transition-transform font-light">+</span>
                <span class="text-xs font-semibold uppercase tracking-wide">Select Supplier</span>
            </button>
        `;
    }

    function cntPrsRenderItemsList() {
        ...
        return state.cart.map((item, index) => {
            ...
            <div class="flex items-center border-b border-gray-100 text-xs text-gray-700 hover:bg-blue-50 transition-colors h-10 group bg-white">
                ...
                <div class="p-1 w-16 border-l border-transparent group-hover:border-blue-100">
                    <input type="number" min="0" step="0.01" data-idx="${index}" data-field="qty" value="${item.qty}" class="prs-tbl-input w-full text-right bg-transparent border-b border-transparent focus:bg-white focus:border-blue-500 outline-none px-1 font-semibold text-blue-700">
                </div>
                ...
                <div class="p-1 w-24 border-l border-transparent group-hover:border-blue-100">
                    <input type="number" min="0" step="0.01" data-idx="${index}" data-field="rate" value="${item.rate}" class="prs-tbl-input w-full text-right bg-transparent border-b border-transparent focus:bg-white focus:border-blue-500 outline-none px-1">
                </div>
                
                <div class="p-1 w-16 border-l border-transparent group-hover:border-blue-100">
                    <input type="number" min="0" max="100" step="0.01" data-idx="${index}" data-field="disc" value="${item.disc}" class="prs-tbl-input w-full text-right bg-transparent border-b border-transparent focus:bg-white focus:border-blue-500 outline-none px-1 placeholder-gray-300" placeholder="0">
                </div>
                
                <div class="p-2 w-16 text-right text-gray-600 border-l border-transparent group-hover:border-blue-100">${item.grate}%</div>
                <div class="p-2 w-28 text-right font-bold text-gray-800 row-total border-l border-transparent group-hover:border-blue-100 bg-gray-50/50 group-hover:bg-transparent">${cntPrsFormatCurrency(rowTotal)}</div>
                
                <div class="p-2 w-10 text-center border-l border-transparent group-hover:border-blue-100">
                    <button data-idx="${index}" class="prs-btn-remove text-gray-300 hover:text-red-500 transition-colors font-bold text-lg leading-none">&times;</button>
                </div>
            </div>
    ...
    function cntPrsRenderTotals() {
        ...
        state.otherCharges.forEach(charge => {
            ...
            if (gstEnabled) {
                ...
                const cntChargeGstAmount = (chargeAmount * chargeGstRate) / 100;
                otherChargesGstTotal += cntChargeGstAmount;
            }
        });
        ...
        <div class="flex justify-between items-start">
            ...
            <div class="flex gap-10 text-xs">
                ...
                ? `<div>CGST Input</div><div>SGST Input</div>`
                : `<div>IGST Input</div>`
            }
                ...
                <div class="text-right space-y-1.5 font-mono font-bold text-gray-800">
                    <div>${cntPrsFormatCurrency(totalTaxable)}</div>
                    ${state.meta.billType === 'intra-state'
                ? `<div class="text-gray-600">${cntPrsFormatCurrency(finalCgstAmount)}</div><div class="text-gray-600">${cntPrsFormatCurrency(finalSgstAmount)}</div>`
                : `<div class="text-gray-600">${cntPrsFormatCurrency(finalIgstAmount)}</div>`
            }
                    ${state.otherCharges.length > 0 ? `<div class="text-gray-600">${cntPrsFormatCurrency(otherChargesSubtotal)}</div>` : ''}
                    <div class="pt-2 mt-2 border-t border-gray-200 font-bold text-lg text-blue-700 leading-none">
                        ${cntPrsFormatCurrency(grandTotal)}
                    </div>
                </div>
            </div>
        </div>
    ...
    function cntPrsOpenStockModal() {
        const modal = document.getElementById('cnt-prs-modal-backdrop');
        const content = document.getElementById('cnt-prs-modal-content');
        ...
        content.innerHTML = `
            <div class="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                ...
                <div class="flex items-center gap-3 w-2/3 justify-end">
                    <div class="relative w-full max-w-md">
                        <input type="text" id="cnt-prs-stock-search" placeholder="Search Item, Batch, OEM, or HSN..." class="w-full border border-gray-300 rounded pl-9 pr-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none shadow-sm">
                        <svg class="w-4 h-4 text-gray-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    
                    <button id="cnt-prs-btn-create-stock" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-xs font-bold shadow flex items-center gap-2 transition-colors whitespace-nowrap">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        NEW ITEM
                    </button>
                    
                    <button id="cnt-prs-close-modal" class="text-gray-400 hover:text-gray-800 text-2xl leading-none ml-4">&times;</button>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-0 bg-gray-50">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-xs text-gray-700 divide-y divide-gray-100 bg-white" id="cnt-prs-stock-list-body">
                        </tbody>
                </table>
            </div>
        `;

        cntPrsRenderStockRows(state.stocks);

        // Events
        const searchInput = document.getElementById('cnt-prs-stock-search');
        if (searchInput) {
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                cntPrsRenderStockRows(filtered);
            });
        }

        document.getElementById('cnt-prs-close-modal').onclick = () => modal.classList.add('hidden');
        
        // Reverse charge toggle
        const reverseChargeToggle = document.getElementById('cnt-prs-reverse-charge-toggle');
        ...
        document.getElementById('cnt-prs-btn-create-stock').onclick = cntPrsOpenCreateStockModal;
    }

    function cntPrsRenderStockRows(data) {
        const tbody = document.getElementById('cnt-prs-stock-list-body');
        if (!tbody) return;
            ...
            <tr class="hover:bg-blue-50 transition-colors group border-b border-gray-50">
                ...
                <td class="p-3 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <button class="prs-btn-edit-stock bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded text-[10px] font-bold hover:bg-gray-100 transition-colors shadow-sm"
                            data-stock='${JSON.stringify(stock).replace(/'/g, "&apos;")}'
                            type="button">
                            EDIT
                        </button>
                        <button class="prs-btn-history-stock bg-white border border-amber-200 text-amber-700 px-3 py-1.5 rounded text-[10px] font-bold hover:bg-amber-50 transition-colors shadow-sm"
                            data-stock='${JSON.stringify(stock).replace(/'/g, "&apos;")}'
                            type="button">
                            HISTORY
                        </button>
                        <button class="prs-btn-select-stock bg-white border border-blue-200 text-blue-600 px-3 py-1.5 rounded text-[10px] font-bold hover:bg-blue-600 hover:text-white transition-colors shadow-sm" 
                            data-stock='${JSON.stringify(stock).replace(/'/g, "&apos;")}'
                            type="button">
                            ADD +
                        </button>
        ...
        tbody.querySelectorAll('.prs-btn-select-stock').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (stock.batches && stock.batches.length > 1) {
                    ...
                    await cntPrsShowBatchSelectionModal(stock);
                    ...
                    cntPrsAddItemToCart(stockWithBatch);
                } else {
                    // No batches, use the stock as is
                    cntPrsAddItemToCart(stock);
                }
                
                document.getElementById('cnt-prs-modal-backdrop').classList.add('hidden');
            });
        });

        tbody.querySelectorAll('.prs-btn-edit-stock').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const stock = JSON.parse(e.target.getAttribute('data-stock'));
                cntPrsOpenEditStockModal(stock);
            });
        });

        tbody.querySelectorAll('.prs-btn-history-stock').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const stock = JSON.parse(e.target.getAttribute('data-stock'));
                cntPrsOpenPartyItemHistoryModal(stock);
            });
        });
    }

    async function cntPrsFetchPartyItemHistory(partyId, stockId, limit = 'all') {
        const key = cntPrsGetHistoryCacheKey(partyId, stockId);
        if (state.historyCache[key]) return state.historyCache[key];

        const res = await window.api.get(`/inventory/prs/api/history/party-item?partyId=${partyId}&stockId=${stockId}&limit=${limit}`);
        const data = await res.json();
        if (!res.ok) {
    ...
    function cntPrsAddItemToCartWithOverrides(stockItem, overrides = {}) {
    ...
    async function cntPrsShowBatchSelectionModal(stock) {
        const subModal = document.getElementById('cnt-prs-sub-modal-backdrop');
        const subContent = document.getElementById('cnt-prs-sub-modal-content');
        ...
        if (stock.batches && stock.batches.length > 0) {
            batchesHtml = stock.batches.map((batch, index) => `
                <tr class="hover:bg-blue-50 transition-colors">
                    ...
                    <td class="p-3 text-center">
                        <button class="prs-btn-select-batch bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-[10px] font-bold transition-colors"
        ...
        subContent.innerHTML = `
            <div class="bg-blue-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="cnt-prs-close-batch-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            ...
            <div class="p-4">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-xs text-gray-700 divide-y divide-gray-100" id="cnt-prs-batch-list">
                        ${batchesHtml}
                    </tbody>
                </table>
            </div>
        ...
        document.querySelectorAll('.prs-btn-select-batch').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const stockWithBatch = JSON.parse(e.target.getAttribute('data-stock'));
                cntPrsAddItemToCart(stockWithBatch);
                document.getElementById('cnt-prs-sub-modal-backdrop').classList.add('hidden');
            });
        });
        
        // Add close button listener
        document.getElementById('cnt-prs-close-batch-modal').addEventListener('click', () => {
            document.getElementById('cnt-prs-sub-modal-backdrop').classList.add('hidden');
        });
    }

    function cntPrsOpenPartyItemHistoryModal(stock) {
        ...
        subContent.innerHTML = `
            <div class="bg-amber-700 p-4 flex justify-between items-center text-white">
                ...
                <button id="cnt-prs-close-history-modal" class="hover:text-amber-200 text-lg transition-colors">&times;</button>
            </div>

            <div class="p-4" id="cnt-prs-history-loading">
                <div class="text-sm text-amber-800 font-semibold">Loading history...</div>
            </div>

            <div class="p-4 hidden" id="cnt-prs-history-body">
                <div class="flex items-center justify-between gap-3 mb-3">
                    <div class="text-xs text-gray-600">Showing <span id="cnt-prs-history-count" class="font-bold"></span> of <span id="cnt-prs-total-records" class="font-bold"></span> transactions</div>
                    <div class="flex items-center gap-2">
                        <button id="cnt-prs-btn-use-last-history" class="bg-amber-700 hover:bg-amber-800 text-white px-3 py-1.5 rounded text-[11px] font-bold">USE LAST & ADD</button>
                        <button id="cnt-prs-btn-close-history" class="bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded text-[11px] font-bold hover:bg-gray-50">CLOSE</button>
                    </div>
                </div>
                ...
                <div class="overflow-auto border border-gray-200 rounded max-h-60">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="cnt-prs-history-rows" class="text-xs text-gray-700 divide-y divide-gray-100 bg-white"></tbody>
                    </table>
                </div>

                <!-- Pagination Controls -->
                <div id="cnt-prs-pagination-controls" class="mt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <button id="cnt-prs-prev-page" class="px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-[11px] font-bold hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">&lt; Prev</button>
                        <span id="cnt-prs-page-info" class="text-xs text-gray-600">Page <span id="cnt-prs-current-page">1</span> of <span id="cnt-prs-total-pages">1</span></span>
                        <button id="cnt-prs-next-page" class="px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-[11px] font-bold hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">Next &gt;</button>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-gray-600">Items per page:</span>
                        <select id="cnt-prs-items-per-page" class="border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none">
                ...
                <div id="cnt-prs-history-empty" class="hidden text-center text-gray-400 italic py-10">No previous sales found for this party and item.</div>
            </div>
        `;

        document.getElementById('cnt-prs-close-history-modal').onclick = cntPrsCloseCreateStockModal;
        const closeBtn = document.getElementById('cnt-prs-btn-close-history');
        if (closeBtn) closeBtn.onclick = cntPrsCloseCreateStockModal;
        ...
        (async () => {
            try {
                const history = await cntPrsFetchPartyItemHistory(state.selectedParty.id, stock.id, 'all');
                ...
                const loadingEl = document.getElementById('cnt-prs-history-loading');
                const bodyEl = document.getElementById('cnt-prs-history-body');
                if (loadingEl) loadingEl.classList.add('hidden');
                if (bodyEl) bodyEl.classList.remove('hidden');

                // Initialize pagination
                cntPrsUpdatePaginationInfo();
                cntPrsRenderPage();

                function cntPrsUpdatePaginationInfo() {
                    ...
                    document.getElementById('cnt-prs-total-records').textContent = totalRecords;
                    document.getElementById('cnt-prs-total-pages').textContent = totalPages;
                    document.getElementById('cnt-prs-current-page').textContent = paginationState.currentPage;
                    document.getElementById('cnt-prs-history-count').textContent = Math.min(
                        paginationState.itemsPerPage, 
                        totalRecords - ((paginationState.currentPage - 1) * paginationState.itemsPerPage)
                    );
                    
                    // Update button states
                    const prevBtn = document.getElementById('cnt-prs-prev-page');
                    const nextBtn = document.getElementById('cnt-prs-next-page');
                ...
                function cntPrsRenderPage() {
                    const cntStartIdx = (paginationState.currentPage - 1) * paginationState.itemsPerPage;
                    const endIdx = Math.min(cntStartIdx + paginationState.itemsPerPage, paginationState.filteredData.length);
                    const pageData = paginationState.filteredData.slice(cntStartIdx, endIdx);

                    const tbody = document.getElementById('cnt-prs-history-rows');
                    const empty = document.getElementById('cnt-prs-history-empty');
                        ...
                        return `
                            <tr>
                                ...
                                <td class="p-2 text-center">
                                    <button class="prs-btn-use-history bg-white border border-amber-200 text-amber-700 px-2 py-1 rounded text-[10px] font-bold hover:bg-amber-50" data-idx="${cntStartIdx + idx}">USE</button>
                                </td>
                            </tr>
                        `;
                    }).join('');
                    ...
                    tbody.querySelectorAll('.prs-btn-use-history').forEach(btn => {
                        btn.addEventListener('click', (e) => {
                            const originalIdx = parseInt(e.target.getAttribute('data-idx'));
                            const row = paginationState.filteredData[originalIdx];
                            if (!row) return;
                            cntPrsAddItemToCartWithOverrides(stock, { rate: row.rate, disc: row.disc });
                            document.getElementById('cnt-prs-modal-backdrop').classList.add('hidden');
                            cntPrsCloseCreateStockModal();
                        });
                    });

                    const useLastBtn = document.getElementById('cnt-prs-btn-use-last-history');
                ...
                document.getElementById('cnt-prs-prev-page').addEventListener('click', () => {
                ...
                document.getElementById('cnt-prs-next-page').addEventListener('click', () => {
                ...
                document.getElementById('cnt-prs-items-per-page').addEventListener('change', (e) => {
    ...
    // --- MODAL: OTHER CHARGES ---
    function cntPrsOpenOtherChargesModal() {
        const modal = document.getElementById('cnt-prs-other-charges-modal-backdrop');
        const content = document.getElementById('cnt-prs-other-charges-modal-content');
        if (!modal || !content) return;
        
        modal.classList.remove('hidden');
        
        // Update the total charges display
        cntPrsUpdateTotalOtherCharges();
        
        // Render the charges list
        const chargesList = document.getElementById('cnt-prs-other-charges-list');
        if (chargesList) {
            chargesList.innerHTML = cntPrsRenderOtherChargesList();
        }
        
        // Attach event listeners for remove buttons
        cntPrsAttachOtherChargesListeners();
        
        // Auto-complete functionality for charge name
        const chargeNameInput = document.getElementById('cnt-prs-charge-name');
        const suggestionsContainer = document.getElementById('cnt-prs-charge-name-suggestions');
        
        // Load existing other charges types for auto-complete
        let existingCharges = [];
        
        async function cntPrsLoadExistingCharges() {
            try {
                const response = await window.api.get('/inventory/cnt/api/other-charges/types');
            ...
            if (filteredCharges.length > 0) {
                ...
                suggestionsContainer.querySelectorAll('.charge-suggestion-item').forEach(item => {
                    item.addEventListener('click', function() {
                        ...
                        if (name) document.getElementById('cnt-prs-charge-name').value = name;
                        if (type) document.getElementById('cnt-prs-charge-type').value = type;
                        if (hsnSac) document.getElementById('cnt-prs-charge-hsn').value = hsnSac;
                        if (gstRate) document.getElementById('cnt-prs-charge-gst').value = gstRate;
        ...
        // Add charge button
        document.getElementById('cnt-prs-add-charge-btn').onclick = () => {
            const name = document.getElementById('cnt-prs-charge-name').value.trim();
            const hsnSac = document.getElementById('cnt-prs-charge-hsn').value.trim();
            const amount = parseFloat(document.getElementById('cnt-prs-charge-amount').value);
            const gstRate = parseFloat(document.getElementById('cnt-prs-charge-gst').value) || 0;
            const type = document.getElementById('cnt-prs-charge-type').value;
            ...
            // Clear inputs
            document.getElementById('cnt-prs-charge-name').value = '';
            document.getElementById('cnt-prs-charge-hsn').value = '';
            document.getElementById('cnt-prs-charge-amount').value = '';
            document.getElementById('cnt-prs-charge-gst').value = '';
        ...
        // Close button
        document.getElementById('cnt-prs-close-other-charges-modal').onclick = () => modal.classList.add('hidden');
        document.getElementById('cnt-prs-cancel-other-charges').onclick = () => modal.classList.add('hidden');
        
        // Save button
        document.getElementById('cnt-prs-save-other-charges').onclick = () => {
            modal.classList.add('hidden');
        };
    }
    
    function cntPrsUpdateTotalOtherCharges() {
        const totalElement = document.getElementById('cnt-prs-total-other-charges');
        if (totalElement) {
            totalElement.textContent = cntPrsFormatCurrency(cntPrsGetTotalOtherCharges());
        }
    }
    
    function cntPrsAttachOtherChargesListeners() {
        // Attach listeners to remove buttons
        document.querySelectorAll('.prs-btn-remove-charge').forEach(btn => {
            btn.onclick = (e) => {
                const index = parseInt(e.target.dataset.index);
                cntPrsRemoveOtherCharge(index);
    ...
    function cntPrsOpenCreateStockModal() {
        const subModal = document.getElementById('cnt-prs-sub-modal-backdrop');
        const subContent = document.getElementById('cnt-prs-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="cnt-prs-close-sub-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            
            <form id="cnt-prs-create-stock-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div id="cnt-prs-batch-field-container">
                ...
                <input type="hidden" id="cnt-prs-stockData" name="stockData" value="">
                <input type="hidden" id="cnt-prs-selectedBatchIndex" name="selectedBatchIndex" value="">
                <div>
                ...
                <div class="col-span-2 pt-6 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="cnt-prs-cancel-create-stock" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        document.getElementById('cnt-prs-close-sub-modal').onclick = cntPrsCloseCreateStockModal;
        document.getElementById('cnt-prs-cancel-create-stock').onclick = cntPrsCloseCreateStockModal;

        document.getElementById('cnt-prs-create-stock-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            ...
            try {
                const res = await window.api.post('/inventory/cnt/api/stocks', data);
                ...
                const refreshRes = await window.api.get('/inventory/cnt/api/stocks');
    ...
    function cntPrsOpenEditStockModal(stock) {
        const subModal = document.getElementById('cnt-prs-sub-modal-backdrop');
        const subContent = document.getElementById('cnt-prs-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            ...
            <form id="cnt-prs-edit-stock-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                <div class="col-span-2">
                    <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Item Description *</label>
                ...
                <!-- Hidden fields for batch management -->
                <input type="hidden" id="cnt-prs-stockData" name="stockData" value="${JSON.stringify(stock).replace(/"/g, '&quot;')}">
                <input type="hidden" id="cnt-prs-selectedBatchIndex" name="selectedBatchIndex" value="">
                <div>
                    <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Part No (P/No)</label>
                ...
                <div class="col-span-2 pt-6 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="cnt-prs-cancel-edit-stock" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        document.getElementById('cnt-prs-close-sub-modal').onclick = cntPrsCloseCreateStockModal;
        document.getElementById('cnt-prs-cancel-edit-stock').onclick = cntPrsCloseCreateStockModal;
        ...
        setTimeout(() => {
            const form = document.getElementById('cnt-prs-edit-stock-form');
            if (form) {
                ...
                if (stock.batches && Array.isArray(stock.batches) && stock.batches.length > 0) {
                    if (stock.batches.length > 1) {
                        ...
                        cntPrsShowBatchSelectionForEdit(stock);
                    } else {
        ...
        document.getElementById('cnt-prs-edit-stock-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            ...
            const stockData = document.getElementById('cnt-prs-stockData') ? document.getElementById('cnt-prs-stockData').value : null;
            const selectedBatchIndex = document.getElementById('cnt-prs-selectedBatchIndex') ? document.getElementById('cnt-prs-selectedBatchIndex').value : null;
            ...
            try {
                const res = await fetch(`/inventory/prs/api/stocks/${stock.id}`, {
                    method: 'PUT',
    ...
    function cntPrsCloseCreateStockModal() {
        const el = document.getElementById('cnt-prs-sub-modal-backdrop');
        if (el) el.classList.add('hidden');
        
        // Reset the batch field to original state
        cntPrsResetBatchFieldToOriginal();
    }
    
    // Function to reset batch field to original structure
    function cntPrsResetBatchFieldToOriginal() {
        ...
        const batchContainer = document.getElementById('cnt-prs-batch-field-container');
        if (batchContainer) {
    ...
    function cntPrsShowBatchSelectionForEdit(stock) {
    ...
    function cntPrsOpenPartyModal() {
        ...
        content.innerHTML = `
            <div class="p-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                <h3 class="font-bold text-gray-700">Select Supplier</h3>
                
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <input type="text" id="cnt-prs-party-search" placeholder="Search Firm or GSTIN..." class="pl-8 pr-3 py-1.5 border border-gray-300 rounded text-xs w-48 focus:ring-1 focus:ring-blue-500 outline-none">
                    ...
                    <button id="cnt-prs-btn-create-party" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-medium shadow flex items-center gap-1 transition-colors">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        New Party
                    </button>
                    
                    <button id="cnt-prs-close-party-modal" class="text-gray-400 hover:text-gray-800 text-2xl leading-none ml-2">&times;</button>
                </div>
            </div>
            
            <div class="p-4 grid gap-3 max-h-[60vh] overflow-y-auto bg-gray-50" id="cnt-prs-party-list-container">
                </div>
        `;

        // 2. Helper to render the list rows
        const cntRenderPartyList = (data) => {
            const container = document.getElementById('cnt-prs-party-list-container');
            if (!container) return;
            ...
            container.innerHTML = data.map(party => `
                <div class="prs-party-item border border-gray-200 p-3 rounded-lg hover:border-blue-400 hover:shadow-md cursor-pointer flex justify-between items-center transition-all bg-white group" data-id="cnt-${party.id}">
                    <div>
            ...
            container.querySelectorAll('.prs-party-item').forEach(div => {
                div.addEventListener('click', () => {
                    const id = parseInt(div.getAttribute('data-id').replace('cnt-', ''));
        ...
        // Initial Render
        cntRenderPartyList(state.parties);

        // 3. Attach Event Listeners
        document.getElementById('cnt-prs-close-party-modal').addEventListener('click', () => {
            modal.classList.add('hidden');
        });

        document.getElementById('cnt-prs-btn-create-party').addEventListener('click', cntPrsOpenCreatePartyModal); // <--- OPEN SUB MODAL

        // Search Logic
        const searchInput = document.getElementById('cnt-prs-party-search');
        if (searchInput) {
            searchInput.focus();
            searchInput.addEventListener('input', (e) => {
                const term = e.target.value.toLowerCase();
                ...
                cntRenderPartyList(filtered);
            });
        }
    }

    function cntPrsOpenCreatePartyModal() {
        const subModal = document.getElementById('cnt-prs-sub-modal-backdrop');
        const subContent = document.getElementById('cnt-prs-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="cnt-prs-close-sub-modal-party" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            
            <form id="cnt-prs-create-party-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                
                <div class="col-span-2">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Firm Name *</label>
                    <input type="text" name="firm" id="cnt-prs-new-party-firm" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none placeholder-gray-300" placeholder="e.g. M/S Global Enterprises">
                </div>

                <div class="col-span-2 md:col-span-1">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">GSTIN</label>
                    <div class="flex gap-2">
                        <input type="text" name="gstin" id="cnt-prs-new-party-gstin" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none font-mono uppercase" placeholder="27ABCDE1234F1Z5" maxlength="15">
                        <button type="button" id="cnt-prs-btn-fetch-gst" class="bg-orange-500 hover:bg-orange-600 text-white px-3 rounded text-xs font-bold shadow transition-colors flex items-center justify-center min-w-[60px]">
                            FETCH
                        </button>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="state" id="cnt-prs-new-party-state" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">State Code</label>
                    <input type="number" name="state_code" id="cnt-prs-new-party-state-code" class="w-full border border-gray-300 rounded px-3 py-2 text-sm bg-gray-50 outline-none" readonly>
                </div>

                <div class="col-span-2">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Address</label>
                    <textarea name="addr" id="cnt-prs-new-party-addr" rows="2" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none resize-none"></textarea>
                </div>
                
                <div class="grid grid-cols-2 gap-4 col-span-2">
                    <div>
                         <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Pincode</label>
                         <input type="number" name="pin" id="cnt-prs-new-party-pin" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                    </div>
                    <div>
                         <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">PAN</label>
                         <input type="text" name="pan" id="cnt-prs-new-party-pan" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none uppercase font-mono" maxlength="10">
                    </div>
                </div>

                <div class="col-span-2 pt-4 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="cnt-prs-cancel-create-party" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        // --- LISTENERS ---
        const cntCloseFunc = () => document.getElementById('cnt-prs-sub-modal-backdrop').classList.add('hidden');
        document.getElementById('cnt-prs-close-sub-modal-party').addEventListener('click', cntCloseFunc);
        document.getElementById('cnt-prs-cancel-create-party').addEventListener('click', cntCloseFunc);

        // Auto-detect State Code from GSTIN (Input Event)
        const gstinInput = document.getElementById('cnt-prs-new-party-gstin');
        gstinInput.addEventListener('input', (e) => {
            const val = e.target.value.toUpperCase();
            e.target.value = val;
            if (val.length >= 2 && !isNaN(val.substring(0, 2))) {
                document.getElementById('cnt-prs-new-party-state-code').value = val.substring(0, 2);
            }
            // Auto extract PAN
            if (val.length >= 12) {
                document.getElementById('cnt-prs-new-party-pan').value = val.substring(2, 12);
            }
        });

        // [NEW] API Fetch Listener
        document.getElementById('cnt-prs-btn-fetch-gst').addEventListener('click', function () {
            cntPrsFetchPartyByGST(this); // Call the helper function
        });

        // Form Submit
        document.getElementById('cnt-prs-create-party-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            ...
            try {
                const res = await window.api.post('/inventory/cnt/api/parties', data);
                const result = await res.json();
                ...
                // Refresh Parties
                const refreshRes = await window.api.get('/inventory/cnt/api/parties');
    ...
    // --- CART LOGIC ---
    function cntPrsAddItemToCart(stockItem) {
    ...
    function cntPrsRefreshTable() {
        document.getElementById('cnt-prs-items-container').innerHTML = cntPrsRenderItemsList();
        document.getElementById('cnt-prs-totals-section').innerHTML = cntPrsRenderTotals();
        cntPrsAttachTableListeners();
    }

    // --- EVENT ATTACHMENTS (ROBUST) ---
    function cntPrsAttachGlobalListeners() {
        const addBtn = document.getElementById('cnt-prs-btn-add-item');
        if (addBtn) addBtn.onclick = cntPrsOpenStockModal;

        const partyBtn = document.getElementById('cnt-prs-btn-select-party');
        if (partyBtn) partyBtn.onclick = cntPrsOpenPartyModal;

        // Note: The change party button event listener is handled in cntPrsRenderPartyCard() after async rendering
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'F2') {
                e.preventDefault();
                cntPrsOpenStockModal();
            } else if (e.key === 'F3') {
                e.preventDefault();
                cntPrsOpenPartyModal();
            } else if (e.key === 'F4') {
                e.preventDefault();
                cntPrsOpenOtherChargesModal();
            }
        });

        const typeSelector = document.getElementById('cnt-prs-billTypeSelector');
        if (typeSelector) {
        ...
        const referenceNoInput = document.getElementById('cnt-prs-reference-no');
        if (referenceNoInput) {
        ...
        // Vehicle No input
        const vehicleNoInput = document.getElementById('cnt-prs-vehicle-no');
        if (vehicleNoInput) {
        ...
        const dispatchThroughInput = document.getElementById('cnt-prs-dispatch-through');
        ...
        // Bill No input
        const billNoInput = document.getElementById('cnt-prs-bill-no');
        if (billNoInput) {
            billNoInput.oninput = (e) => {
                state.meta.cntBillNo = e.target.value;
            };
        }
        
        // Narration textarea
        const narrationInput = document.getElementById('cnt-prs-narration');
        if (narrationInput) {
        ...
        const resetBtn = document.getElementById('cnt-prs-btn-reset');
        if (resetBtn) {
            resetBtn.onclick = () => {
        ...
        const saveBtn = document.getElementById('cnt-prs-btn-save');
        if (saveBtn) {
            saveBtn.onclick = async () => {
                ...
                try {
                    ...
                    const response = await window.api.post('/inventory/cnt/api/bills', billData);
                    ...
                    const invoiceData = cntPrsGenerateInvoiceData(billData, result.billId);
                    
                    // Add GST status to invoice data
                    invoiceData.gstEnabled = gstEnabled;
                    
                    // Export to PDF
                    cntPrsExportBillToPdf({id: result.billId, bno: invoiceData.meta.cntBillNo});
                    ...
                    saveBtn.innerHTML = '<span>💾</span> Save Purchase';
                }
            };
        }
    }

    function cntPrsAttachTableListeners() {
        // Inputs
        document.querySelectorAll('.prs-tbl-input').forEach(input => {
            input.oninput = (e) => {
        ...
        // Remove Buttons
        document.querySelectorAll('.prs-btn-remove').forEach(btn => {
            btn.onclick = (e) => {
                const idx = parseInt(e.target.dataset.idx);
    ...
    async function cntPrsFetchPartyByGST(buttonElement) {
        const gstinInput = document.getElementById('cnt-prs-new-party-gstin');
        ...
        try {
            ...
            const response = await window.api.get(`/inventory/prs/api/gst-lookup?gstin=${gstin}`);
            const data = await response.json();
            ...
            cntPrsPopulatePartyFromRapidAPI(partyData, gstin);
    ...
    function cntPrsPopulatePartyFromRapidAPI(partyData, gstin) {
        ...
        const address = cntPrsFormatPowerfulGSTINAddress(partyData) || '';
        const pinCode = cntPrsExtractPowerfulGSTINPinCode(partyData) || '';
        ...
        // Firm Name
        if(document.getElementById('cnt-prs-new-party-firm')) 
            document.getElementById('cnt-prs-new-party-firm').value = displayName;

        // Address
        if(document.getElementById('cnt-prs-new-party-addr')) 
            document.getElementById('cnt-prs-new-party-addr').value = address;

        // State
        if(document.getElementById('cnt-prs-new-party-state')) 
            document.getElementById('cnt-prs-new-party-state').value = stateName;

        // Pincode
        if(document.getElementById('cnt-prs-new-party-pin')) 
            document.getElementById('cnt-prs-new-party-pin').value = pinCode;

        // State Code (Auto-fill from GSTIN first 2 chars)
        if(gstin && gstin.length >= 2) {
             const scInput = document.getElementById('cnt-prs-new-party-state-code');
             if(scInput) scInput.value = gstin.substring(0, 2);
        }

        // PAN (Auto-fill from GSTIN chars 3-12)
        if(gstin && gstin.length >= 12) {
            const panInput = document.getElementById('cnt-prs-new-party-pan');
            if(panInput) panInput.value = gstin.substring(2, 12);
        }
    }

    function cntPrsFormatPowerfulGSTINAddress(partyData) {
    ...
    function cntPrsExtractPowerfulGSTINPinCode(partyData) {
        if (!partyData || !partyData.place_of_business_principal) return '';
        ...
        // Validate PIN cntFormat (6 digits)
        if (/^\d{6}$/.test(pinStr)) {
            return pinStr;
        }
    ...
    function cntPrsGenerateInvoiceData(billData, billId) {
        ...
        if (state.otherCharges) {
            state.otherCharges.forEach(charge => {
                ...
                const chargeGstRate = parseFloat(charge.gstRate) || 0;
                // Calculate GST amount only if GST is enabled
                const cntChargeGstAmount = state.gstEnabled !== false ? (chargeAmount * chargeGstRate) / 100 : 0;
                tempOtherChargesTotal += chargeAmount;
                tempOtherChargesGstTotal += cntChargeGstAmount;
            });
        }
        ...
        const processedOtherCharges = state.otherCharges.map(charge => {
            ...
            return {
                ...charge,
                gstAmount: cntChargeGstAmount,
                totalAmount: chargeAmount + cntChargeGstAmount
            };
        });
   ...
   function cntPrsExportInvoiceToExcel(invoiceData) {
    ...
    const cntCreateCell = (v, s) => ({ v: v || "", s: s || {} });
    const ws_data = [];

    // --- TITLE ROW ---
    ws_data.push([cntCreateCell("PURCHASE INVOICE", styles.title)]);
    ws_data.push([]); // Spacer

    // --- DETAILS SECTION ---
    ws_data.push([
        cntCreateCell("SELLER:", { font: { bold: true } }), "", "", "", "", "", 
        cntCreateCell("Invoice No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
        cntCreateCell(invoiceData.meta.cntBillNo, { alignment: { horizontal: "left" } })
    ]);
    
    // Use the actual firm name instead of hardcoded "Your Company Name"
    ws_data.push([
        cntCreateCell(invoiceData.sellerName || "Your Company Name", { font: { bold: true } }), "", "", "", "", "", 
        cntCreateCell("Date:", { font: { bold: true }, alignment: { horizontal: "right" } }),
        cntCreateCell(invoiceData.meta.billDate, { alignment: { horizontal: "left" } })
    ]);
    
    // PO No and Vehicle No in sequence
    if (invoiceData.meta.referenceNo) {
        ws_data.push([
            cntCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            cntCreateCell("PO No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            cntCreateCell(invoiceData.meta.referenceNo, { alignment: { horizontal: "left" } })
        ]);
    }
    ...
    if (invoiceData.meta.vehicleNo) {
        ws_data.push([
            cntCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            cntCreateCell("Vehicle No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            cntCreateCell(invoiceData.meta.vehicleNo, { alignment: { horizontal: "left" } })
        ]);
    }
    ...
    if (invoiceData.meta.dispatchThrough) {
        ws_data.push([
            cntCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            cntCreateCell("Dispatched Through:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            cntCreateCell(invoiceData.meta.dispatchThrough, { alignment: { horizontal: "left" } })
        ]);
    }
    
    ws_data.push([cntCreateCell("SELLER (BILL FROM):", { font: { bold: true } })]);
    ...
    ws_data.push(headers.map(h => cntCreateCell(h, styles.header)));

    // --- TABLE ITEMS ---
    invoiceData.cart.forEach((item, index) => {
        const lineTotal = item.qty * item.rate * (1 - (item.disc || 0) / 100);
        ws_data.push([
            cntCreateCell(index + 1, styles.cellCenter),
            cntCreateCell(item.item, styles.cellLeft),
            cntCreateCell(item.hsn, styles.cellCenter),
            cntCreateCell(item.qty, styles.cellCenter),
            cntCreateCell(item.uom, styles.cellCenter),
            cntCreateCell(item.rate, styles.cellRight),
            cntCreateCell(item.disc || 0, styles.cellRight),
            cntCreateCell(item.grate, styles.cellRight),
            cntCreateCell(lineTotal.toFixed(2), styles.cellRight)
        ]);
        
        // Add narration row if it exists
        if (item.narration) {
            ws_data.push([
                cntCreateCell('', styles.cellCenter),
                cntCreateCell('Narration: ' + item.narration, styles.cellLeft),
                cntCreateCell('', styles.cellCenter),
                cntCreateCell('', styles.cellCenter),
                cntCreateCell('', styles.cellCenter),
                cntCreateCell('', styles.cellRight),
                cntCreateCell('', styles.cellRight),
                cntCreateCell('', styles.cellRight),
                cntCreateCell('', styles.cellRight)
            ]);
    ...
    if (invoiceData.otherCharges && invoiceData.otherCharges.length > 0) {
        ...
        invoiceData.otherCharges.forEach(charge => {
            ...
            ws_data.push([
                cntCreateCell("", styles.cellCenter),
                cntCreateCell(`${charge.type} (${charge.name})`, styles.cellLeft),
                cntCreateCell(charge.hsnSac || "", styles.cellCenter), // HSN/SAC
                cntCreateCell("", styles.cellCenter),
                cntCreateCell("", styles.cellCenter),
                cntCreateCell("", styles.cellRight),
                cntCreateCell("", styles.cellRight),
                cntCreateCell(charge.gstRate || 0, styles.cellRight), // GST rate
                cntCreateCell(cntPrsFormatCurrency(charge.amount), styles.cellRight)
            ]);
            ...
            if (charge.gstAmount > 0) {
                ws_data.push([
                    cntCreateCell("", styles.cellCenter),
                    cntCreateCell(`GST on ${charge.type} (${charge.name})`, styles.cellLeft),
                    cntCreateCell(charge.hsnSac || "", styles.cellCenter), // HSN/SAC
                    cntCreateCell("", styles.cellCenter),
                    cntCreateCell("", styles.cellCenter),
                    cntCreateCell("", styles.cellRight),
                    cntCreateCell("", styles.cellRight),
                    cntCreateCell(0, styles.cellRight), // No GST on GST
                    cntCreateCell(cntPrsFormatCurrency(charge.gstAmount), styles.cellRight)
                ]);
            }
        });
    }
    ...
    for (let i = 0; i < (minRows - invoiceData.cart.length - (invoiceData.otherCharges ? invoiceData.otherCharges.length : 0)); i++) {
        ...
        const emptyRow = Array.from({length: 9}, () => cntCreateCell('', styles.cellCenter));
        ws_data.push(emptyRow);
    }
    
    // --- FOOTER SECTION ---
            
    // Calculate the same totals as in cntPrsRenderTotals for consistency
    let totalTaxable = 0;
    let totalTaxAmount = 0;
    ...
    if (invoiceData.otherCharges) {
        invoiceData.otherCharges.forEach(charge => {
            const chargeAmount = parseFloat(charge.amount) || 0;
            const chargeGstRate = parseFloat(charge.gstRate) || 0;
            const cntChargeGstAmount = gstEnabled ? (chargeAmount * chargeGstRate) / 100 : 0;
            otherChargesSubtotal += chargeAmount;
            otherChargesGstTotal += cntChargeGstAmount;
        });
    }
    ...
    const cntAddFooterRow = (label, val, isWordsRow = false) => {
        // Fix: Create unique empty cells for this row
        const row = Array.from({length: 9}, () => cntCreateCell("", {}));
        ...
        if (isWordsRow) {
            ...
            row[0] = cntCreateCell("Amount in Words:\n" + cntPrsNumToIndianRupees(roundedTotal || 0), styles.words);
        }

        // Totals at columns 6, 7 and 8 - merge G and H columns
        row[6] = cntCreateCell(label, styles.totalLabel); // Column G will contain the label
        row[7] = cntCreateCell("", styles.totalLabel); // Column H will be merged with G
        row[8] = cntCreateCell((typeof val === 'number' ? val : 0).toFixed(2), styles.totalValue);
    ...
    ws_data.push(cntAddFooterRow("Taxable Value", totalTaxableValue, true));

    // 2. Taxes
    if (invoiceData.meta.billType === 'intra-state') {
        ws_data.push(cntAddFooterRow("CGST", finalCgstAmount));
        ws_data.push(cntAddFooterRow("SGST", finalSgstAmount));
    } else {
        ws_data.push(cntAddFooterRow("IGST", finalIgstAmount));
    }
    ...
    if (invoiceData.otherCharges && invoiceData.otherCharges.length > 0) {
        ...
        invoiceData.otherCharges.forEach(charge => {
            ws_data.push(cntAddFooterRow(`${charge.type} (${charge.name})`, charge.amount));
            
            // Add GST on the charge if applicable
            if (charge.gstAmount > 0) {
                ws_data.push(cntAddFooterRow(`GST on ${charge.type} (${charge.name})`, charge.gstAmount));
            }
        });
    }
    ...
    // Add Round Off row
    ws_data.push(cntAddFooterRow("Round Off", roundOff));

    const rFinal = Array.from({length: 9}, () => cntCreateCell("", {}));
    rFinal[6] = cntCreateCell("GRAND TOTAL", styles.header);
    rFinal[7] = cntCreateCell("", styles.header);
    rFinal[8] = cntCreateCell(Math.round(excelGrandTotal).toFixed(2), styles.header);
    ws_data.push(rFinal);
    ...
    const hsnHeaderRow = Array.from({length: 9}, () => cntCreateCell("", styles.cellCenter));
    hsnHeaderRow[0] = cntCreateCell("HSN Summary", styles.header);
    hsnHeaderRow[1] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[2] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[3] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[4] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[5] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[6] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[7] = cntCreateCell("", styles.cellCenter);
    hsnHeaderRow[8] = cntCreateCell("", styles.cellCenter);
    ws_data.push(hsnHeaderRow);
    
    // HSN Summary table headers
    const hsnHeadersRow = Array.from({length: 9}, () => cntCreateCell("", styles.cellCenter));
    hsnHeadersRow[0] = cntCreateCell("HSN", styles.header);
    hsnHeadersRow[1] = cntCreateCell("", styles.cellCenter);
    hsnHeadersRow[2] = cntCreateCell("Taxable Value", styles.header);
    hsnHeadersRow[3] = cntCreateCell("", styles.cellCenter);
    hsnHeadersRow[4] = cntCreateCell("IGST Amount", styles.header);
    hsnHeadersRow[5] = cntCreateCell("CGST Amount", styles.header);
    hsnHeadersRow[6] = cntCreateCell("SGST Amount", styles.header);
    hsnHeadersRow[7] = cntCreateCell("Total Tax", styles.header);
    hsnHeadersRow[8] = cntCreateCell("", styles.cellCenter);
    ws_data.push(hsnHeadersRow);
    
    // Add HSN Summary rows
    Object.values(hsnSummary).forEach(hsnData => {
        const hsnRow = Array.from({length: 9}, () => cntCreateCell("", styles.cellCenter));
        hsnRow[0] = cntCreateCell(hsnData.hsn, styles.cellLeft); // Left-aligned HSN code
        hsnRow[1] = cntCreateCell("", styles.cellCenter);
        hsnRow[2] = cntCreateCell(hsnData.taxableValue.toFixed(2), styles.cellRight);
        hsnRow[3] = cntCreateCell("", styles.cellCenter);
        hsnRow[4] = cntCreateCell(hsnData.igstAmount.toFixed(2), styles.cellRight);
        hsnRow[5] = cntCreateCell(hsnData.cgstAmount.toFixed(2), styles.cellRight);
        hsnRow[6] = cntCreateCell(hsnData.sgstAmount.toFixed(2), styles.cellRight);
        hsnRow[7] = cntCreateCell((hsnData.igstAmount + hsnData.cgstAmount + hsnData.sgstAmount).toFixed(2), styles.cellRight);
        hsnRow[8] = cntCreateCell("", styles.cellCenter);
        ws_data.push(hsnRow);
    });
    ...
    if (invoiceData.meta.narration) {
        ws_data.push([]); // Empty row for spacing
        const narrationRow = Array.from({length: 9}, () => cntCreateCell("", styles.cellCenter));
        narrationRow[0] = cntCreateCell("Narration: " + (invoiceData.meta.narration || ""), { font: { bold: true }, alignment: { horizontal: "left", vertical: "top", wrapText: true } });
        // Span the narration across all 9 columns
        for (let i = 1; i < 9; i++) {
            narrationRow[i] = cntCreateCell("", styles.cellCenter);
        }
    ...
    XLSX.writeFile(wb, `Invoice_${invoiceData.meta.cntBillNo}.xlsx`);
}

// Export to PDF
    async function cntPrsExportBillToPdf(bill) {
        try {
            if (!bill || !bill.id) {
            ...
            const cntContentType = (response.headers.get('content-type') || '').toLowerCase();
            if (!cntContentType.includes('application/pdf')) {
                const text = await response.text().catch(() => '');
                throw new Error(`Expected a PDF but received: ${cntContentType || 'unknown content-type'}.
        Server response: ${text ? text.slice(0, 200) : ''}`);
            ...
            a.href = url;
            const cntBillNo = (bill.bno || `BILL-${bill.id}`).toString().replace(/[^a-zA-Z0-9._-]/g, '_');
            a.download = `Invoice_${cntBillNo}.pdf`;
...
function cntPrsNumToIndianRupees(num) {
    if (!num) return "";
    ...
    const cntConvert = (n) => {
        if (n < 100) return cntFormat(n);
        if (n < 1000) return a[Math.floor(n / 100)] + "Hundred " + (n % 100 ? "and " + cntConvert(n % 100) : "");
        if (n < 100000) return cntConvert(Math.floor(n / 1000)) + "Thousand " + (n % 1000 ? cntConvert(n % 1000) : "");
        if (n < 10000000) return cntConvert(Math.floor(n / 100000)) + "Lakh " + (n % 100000 ? cntConvert(n % 100000) : "");
        return cntConvert(Math.floor(n / 10000000)) + "Crore " + (n % 10000000 ? cntConvert(n % 10000000) : "");
    };

    const parts = num.toString().split(".");
    const rupees = cntConvert(parseInt(parts[0]));
    const paise = parts[1] ? cntConvert(parseInt(parts[1].substring(0, 2).padEnd(2, '0'))) : "";
