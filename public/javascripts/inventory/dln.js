(function dlnInitSalesSystem() {
    ...
    async function dlnFetchCurrentUserFirmName() {
        try {
            const response = await window.api.get('/inventory/dln/api/current-user-firm-name');
    ...
    function dlnUpdateConsigneeDisplay() {
        ...
        const pinEl = document.getElementById('dln-consignee-pin');
    ...
    function dlnRemoveOtherCharge(index) {
        state.otherCharges.splice(index, 1);
        dlnRefreshTable();
    }
    
    function dlnUpdateOtherCharge(index, charge) {
        // Calculate GST amount only if GST is enabled
        if (state.gstEnabled !== false) {  // Default to enabled if not set
    ...
    function dlnGetTotalOtherCharges() {
    ...
    async function dlnFetchNextBillNumber() {
        try {
            const billNoRes = await window.api.get('/inventory/dln/api/bills/next-number');
            ...
            if (typeof billNoData.nextBillNo === 'string') {
                state.meta.dlnBillNo = billNoData.nextBillNo;
            } else {
                console.warn('Invalid bill number data received, using placeholder', billNoData);
                state.meta.dlnBillNo = 'Will be generated on save';
            }
        } catch (e) {
    ...
    async function dlnFetchData() {
        try {
            // 1. Fetch Stocks
            const res = await window.api.get('/inventory/dln/api/stocks');
            ...
            // 2. Fetch Parties
            try {
                const partyRes = await window.api.get('/inventory/dln/api/parties');
    ...
    function dlnRenderLayout() {
        ...
        <div class="h-[calc(100vh-140px)] flex flex-col bg-gray-50 text-slate-800 font-sans text-sm border border-gray-300 rounded-lg shadow-sm overflow-hidden">
            ...
            <div class="bg-white border-b border-gray-200 p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 shadow-sm z-20">
                <div class="flex flex-col sm:flex-row flex-wrap gap-2">
                    <div class="flex flex-col">
                        ...
                        <input type="text" value="${state.meta.dlnBillNo}" readonly class="border border-gray-300 rounded px-2 py-1 text-xs font-bold w-32 bg-gray-100 text-slate-500 cursor-not-allowed" title="Auto-generated when saved">
                    </div>
                    <div class="flex flex-col">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select id="dln-billTypeSelector" class="border border-gray-300 rounded px-2 py-1 text-xs bg-white focus:ring-1 focus:ring-blue-500 outline-none text-slate-700 font-medium">
                    ...
                    <div class="flex items-center gap-2 pt-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" id="dln-reverse-charge-toggle" ${state.meta.reverseCharge ? 'checked' : ''} class="form-checkbox h-4 w-4 text-blue-600 rounded">
                ...
                <div class="flex flex-wrap gap-2">
                    <button id="dln-btn-other-charges" class="px-3 py-1.5 text-xs text-blue-600 border border-blue-200 bg-blue-50 rounded hover:bg-blue-100 transition-colors whitespace-nowrap">Other Charges</button>
                    <button id="dln-btn-reset" class="px-3 py-1.5 text-xs text-red-600 border border-red-200 bg-red-50 rounded hover:bg-red-100 transition-colors whitespace-nowrap">Reset</button>
                    <button id="dln-btn-save" class="px-4 py-1.5 bg-slate-800 text-white text-xs rounded hover:bg-slate-900 shadow font-medium flex items-center gap-2 transition-colors whitespace-nowrap">
            ...
            <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
                ...
                <div class="w-full md:w-64 bg-slate-50 border-r border-gray-200 flex flex-col overflow-y-auto z-10">
                            ...
                            <div class="group bg-blue-50 p-3 rounded border border-blue-200 shadow-sm">
                                <div>
                                    ...
                                    <div class="mt-2 text-right">
                                        <button id="dln-btn-change-party" class="text-[10px] text-blue-600 hover:text-blue-800 font-bold bg-white px-2 py-1 rounded shadow-sm border border-gray-200 hover:border-blue-300 whitespace-nowrap">
                                            Change
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div id="dln-consignee-display">
                            <div class="space-y-2">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="dln-consignee-pin" value="${state.selectedConsignee?.pin || ''}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="Enter PIN code" maxlength="6">
                                </div>
                                <div>
                    ...
                    <div class="p-3 space-y-3">
                         <div>
                            ...
                            <input type="text" id="dln-reference-no" value="${state.meta.referenceNo}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="e.g. PO-2025-001">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Vehicle No</label>
                            <input type="text" id="dln-vehicle-no" value="${state.meta.vehicleNo}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="XX-00-XX-0000">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Dispatched Through</label>
                            <input type="text" id="dln-dispatch-through" value="${state.meta.dispatchThrough}" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none" placeholder="Courier / Transport">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 font-bold">Narration</label>
                            <textarea id="dln-narration" class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none h-20 resize-none" placeholder="Additional notes...">${state.meta.narration}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 bg-white flex flex-col relative min-w-0">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar relative" id="dln-items-container">
                        ${dlnRenderItemsList()}
                    </div>

                    <div class="p-2 border-t border-dashed border-gray-200 bg-gray-50 shrink-0">
                        <button id="dln-btn-add-item" class="w-full py-2 border border-dashed border-blue-300 text-blue-600 rounded hover:bg-blue-50 text-xs font-bold transition-colors uppercase tracking-wide">
                            + Add Items (F2) | Select Party (F3) | Charges (F4)
                        </button>
                    </div>

                    <div class="bg-slate-50 border-t border-slate-300 p-4 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]" id="dln-totals-section">
                        ${dlnRenderTotals()}
                    </div>
                </div>
            </div>
        ...
        <div id="dln-modal-backdrop" class="fixed inset-0 bg-black/50 hidden z-40 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="dln-modal-content" class="bg-white rounded shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-down">
                </div>
        </div>

        <div id="dln-sub-modal-backdrop" class="fixed inset-0 bg-black/60 hidden z-50 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="dln-sub-modal-content" class="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-300 animate-scale-in">
                </div>
        </div>
        
        <!-- Other Charges Modal -->
        <div id="dln-other-charges-modal-backdrop" class="fixed inset-0 bg-black/60 hidden z-50 flex items-center justify-center backdrop-blur-sm transition-opacity">
            <div id="dln-other-charges-modal-content" class="bg-white rounded-lg shadow-2xl w-full max-w-3xl overflow-hidden border border-gray-300 animate-scale-in">
                <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                    <h3 class="font-bold text-sm tracking-wide">OTHER CHARGES</h3>
                    <button id="dln-close-other-charges-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
                </div>
                
                <div class="p-6">
                    <div class="flex gap-4 mb-4">
                        <div class="relative flex-1">
                            <input type="text" id="dln-charge-name" placeholder="Charge Name" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                            <div id="dln-charge-name-suggestions" class="absolute z-10 w-full min-w-[400px] bg-white border border-gray-300 rounded shadow-lg mt-1 max-h-40 overflow-y-auto hidden"></div>
                        </div>
                        <input type="text" id="dln-charge-hsn" placeholder="HSN/SAC" class="w-24 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none" title="Enter HSN for goods or SAC for services">
                        <input type="number" id="dln-charge-amount" placeholder="Amount" step="0.01" class="w-32 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                        <input type="number" id="dln-charge-gst" placeholder="GST %" step="0.01" class="w-24 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                        <select id="dln-charge-type" class="w-40 border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none bg-white">
                        ...
                        <button id="dln-add-charge-btn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors whitespace-nowrap">
                            ADD
                        </button>
                    </div>
                    ...
                    <div class="mb-4 text-sm text-gray-600">
                        ...
                        <span id="dln-total-other-charges" class="font-bold text-blue-700">0.00</span>
                    </div>
                    ...
                    <div class="overflow-y-auto max-h-80">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody id="dln-other-charges-list" class="text-xs text-gray-700 divide-y divide-gray-100">
                                ${dlnRenderOtherChargesList()}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="p-4 border-t border-gray-200 flex justify-end gap-3">
                    <button id="dln-cancel-other-charges" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
                    <button id="dln-save-other-charges" class="px-6 py-2 bg-green-600 text-white text-sm font-bold rounded shadow hover:bg-green-700 transition-colors">DONE</button>
                </div>
            </div>
        </div>
        `;
        ...
        const partyContainer = document.getElementById('dln-party-display');
        if (partyContainer) {
            dlnRenderPartyCard().then(html => {
                partyContainer.innerHTML = html;
                
                // Attach event listeners for both change and select buttons
                const changeBtn = document.getElementById('dln-btn-change-party');
                if (changeBtn) changeBtn.addEventListener('click', dlnOpenPartyModal);
                
                const selectBtn = document.getElementById('dln-btn-select-party');
                if (selectBtn) selectBtn.addEventListener('click', dlnOpenPartyModal);
            });
        }
    ...
    function dlnRenderOtherChargesList() {
        ...
        return state.otherCharges.map((charge, index) => {
            ...
            <tr class="hover:bg-blue-50 transition-colors">
                ...
                <td class="p-3 text-right font-bold text-gray-800">${dlnFormatCurrency(charge.amount)}</td>
            ...
            <tr class="hover:bg-blue-50 transition-colors bg-gray-50">
                ...
                <td class="p-1 text-right text-gray-500 text-xs">${dlnFormatCurrency(gstAmount)}</td>
                <td class="p-1 text-right text-gray-500 text-xs font-bold">Total:</td>
                <td class="p-1 text-right text-gray-800 text-xs font-bold">${dlnFormatCurrency(totalAmount)}</td>
            </tr>`;
        }).join('');
    }
    
    async function dlnRenderPartyCard() {
        if (state.selectedParty) {
            // Fetch party balance
                ...
                <div class="group bg-blue-50 p-3 rounded border border-blue-200 shadow-sm">
                    <div>
                        <div class="flex justify-between items-start">
                            ...
                            <button id="dln-btn-change-party" class="text-[10px] text-blue-600 hover:text-blue-800 font-bold bg-white p-1.5 rounded shadow-sm border border-gray-200 hover:border-blue-300 whitespace-nowrap ml-2" title="Change Party">
        ...
        return `
            <button id="dln-btn-select-party" class="w-full py-6 border-2 border-dashed border-gray-300 rounded-lg text-gray-400 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-all flex flex-col items-center justify-center gap-2 group">
    ...
    function dlnRenderItemsList() {
        ...
        return state.cart.map((item, index) => {
            ...
            <div class="flex items-center border-b border-gray-100 text-xs text-gray-700 hover:bg-blue-50 transition-colors h-10 group bg-white">
                ...
                <div class="p-2 w-28 text-right font-bold text-gray-800 row-total border-l border-transparent group-hover:border-blue-100 bg-gray-50/50 group-hover:bg-transparent">${dlnFormatCurrency(rowTotal)}</div>
    ...
    function dlnRenderTotals() {
        ...
        state.otherCharges.forEach(charge => {
            ...
            if (gstEnabled) {
                ...
                const dlnChargeGstAmount = (chargeAmount * chargeGstRate) / 100;
                otherChargesGstTotal += dlnChargeGstAmount;
            }
        });
        ...
        <div class="flex justify-between items-start">
            ...
            <div class="flex gap-10 text-xs">
                ...
                <div class="text-right space-y-1.5 font-mono font-bold text-gray-800">
                    <div>${dlnFormatCurrency(totalTaxable)}</div>
                    ${state.meta.billType === 'intra-state'
                ? `<div class="text-gray-600">${dlnFormatCurrency(finalCgstAmount)}</div><div class="text-gray-600">${dlnFormatCurrency(finalSgstAmount)}</div>`
                : `<div class="text-gray-600">${dlnFormatCurrency(finalIgstAmount)}</div>`
            }
                    ${state.otherCharges.length > 0 ? `<div class="text-gray-600">${dlnFormatCurrency(otherChargesSubtotal)}</div>` : ''}
                    <div class="pt-2 mt-2 border-t border-gray-200 font-bold text-lg text-blue-700 leading-none">
                        ${dlnFormatCurrency(grandTotal)}
                    </div>
                </div>
            </div>
        </div>
    ...
    function dlnOpenStockModal() {
        const modal = document.getElementById('dln-modal-backdrop');
        const content = document.getElementById('dln-modal-content');
        ...
        content.innerHTML = `
            <div class="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                ...
                <div class="flex items-center gap-3 w-2/3 justify-end">
                    <div class="relative w-full max-w-md">
                        <input type="text" id="dln-stock-search" placeholder="Search Item, Batch, OEM, or HSN..." class="w-full border border-gray-300 rounded pl-9 pr-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none shadow-sm">
                        <svg class="w-4 h-4 text-gray-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    
                    <button id="dln-btn-create-stock" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-xs font-bold shadow flex items-center gap-2 transition-colors whitespace-nowrap">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        NEW ITEM
                    </button>
                    
                    <button id="dln-close-modal" class="text-gray-400 hover:text-gray-800 text-2xl leading-none ml-4">&times;</button>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-0 bg-gray-50">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-xs text-gray-700 divide-y divide-gray-100 bg-white" id="dln-stock-list-body">
                        </tbody>
                </table>
            </div>
        `;

        dlnRenderStockRows(state.stocks);

        // Events
        const searchInput = document.getElementById('dln-stock-search');
        if (searchInput) {
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                dlnRenderStockRows(filtered);
            });
        }

        document.getElementById('dln-close-modal').onclick = () => modal.classList.add('hidden');
        
        // Reverse charge toggle
        const reverseChargeToggle = document.getElementById('dln-reverse-charge-toggle');
        ...
        document.getElementById('dln-btn-create-stock').onclick = dlnOpenCreateStockModal;
    }

    function dlnRenderStockRows(data) {
        const tbody = document.getElementById('dln-stock-list-body');
        if (!tbody) return;
        ...
        tbody.querySelectorAll('.btn-select-stock').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (stock.batches && stock.batches.length > 1) {
                    ...
                    await dlnShowBatchSelectionModal(stock);
                    ...
                    dlnAddItemToCart(stockWithBatch);
                } else {
                    // No batches, use the stock as is
                    dlnAddItemToCart(stock);
                }
                
                document.getElementById('dln-modal-backdrop').classList.add('hidden');
            });
        });
        ...
        tbody.querySelectorAll('.btn-history-stock').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const stock = JSON.parse(e.target.getAttribute('data-stock'));
                dlnOpenPartyItemHistoryModal(stock);
            });
        });
    }

    async function dlnFetchPartyItemHistory(partyId, stockId, limit = 'all') {
        const key = dlnGetHistoryCacheKey(partyId, stockId);
    ...
    function dlnAddItemToCartWithOverrides(stockItem, overrides = {}) {
    ...
    async function dlnShowBatchSelectionModal(stock) {
        const subModal = document.getElementById('dln-sub-modal-backdrop');
        const subContent = document.getElementById('dln-sub-modal-content');
        ...
        subContent.innerHTML = `
            <div class="bg-blue-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="dln-close-batch-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            ...
            <div class="p-4">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-xs text-gray-700 divide-y divide-gray-100" id="dln-batch-list">
                        ${batchesHtml}
                    </tbody>
                </table>
            </div>
        ...
        document.querySelectorAll('.btn-select-batch').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                dlnAddItemToCart(stockWithBatch);
                document.getElementById('dln-sub-modal-backdrop').classList.add('hidden');
            });
        });
        
        // Add close button listener
        document.getElementById('dln-close-batch-modal').addEventListener('click', () => {
            document.getElementById('dln-sub-modal-backdrop').classList.add('hidden');
        });
    }

    function dlnOpenPartyItemHistoryModal(stock) {
        ...
        subContent.innerHTML = `
            <div class="bg-amber-700 p-4 flex justify-between items-center text-white">
                ...
                <button id="dln-close-history-modal" class="hover:text-amber-200 text-lg transition-colors">&times;</button>
            </div>

            <div class="p-4" id="dln-history-loading">
                <div class="text-sm text-amber-800 font-semibold">Loading history...</div>
            </div>

            <div class="p-4 hidden" id="dln-history-body">
                <div class="flex items-center justify-between gap-3 mb-3">
                    <div class="text-xs text-gray-600">Showing <span id="dln-history-count" class="font-bold"></span> of <span id="dln-total-records" class="font-bold"></span> transactions</div>
                    <div class="flex items-center gap-2">
                        <button id="dln-btn-use-last-history" class="bg-amber-700 hover:bg-amber-800 text-white px-3 py-1.5 rounded text-[11px] font-bold">USE LAST & ADD</button>
                        <button id="dln-btn-close-history" class="bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded text-[11px] font-bold hover:bg-gray-50">CLOSE</button>
                    </div>
                </div>
                ...
                <div class="overflow-auto border border-gray-200 rounded max-h-60">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="dln-history-rows" class="text-xs text-gray-700 divide-y divide-gray-100 bg-white"></tbody>
                    </table>
                </div>

                <!-- Pagination Controls -->
                <div id="dln-pagination-controls" class="mt-4 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <button id="dln-prev-page" class="px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-[11px] font-bold hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">&lt; Prev</button>
                        <span id="dln-page-info" class="text-xs text-gray-600">Page <span id="dln-current-page">1</span> of <span id="dln-total-pages">1</span></span>
                        <button id="dln-next-page" class="px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-[11px] font-bold hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">Next &gt;</button>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-gray-600">Items per page:</span>
                        <select id="dln-items-per-page" class="border border-gray-300 rounded px-2 py-1 text-xs focus:border-blue-500 outline-none">
                ...
                <div id="dln-history-empty" class="hidden text-center text-gray-400 italic py-10">No previous sales found for this party and item.</div>
            </div>
        `;

        document.getElementById('dln-close-history-modal').onclick = dlnCloseCreateStockModal;
        const closeBtn = document.getElementById('dln-btn-close-history');
        if (closeBtn) closeBtn.onclick = dlnCloseCreateStockModal;
        ...
        (async () => {
            try {
                const history = await dlnFetchPartyItemHistory(state.selectedParty.id, stock.id, 'all');
                ...
                const loadingEl = document.getElementById('dln-history-loading');
                const bodyEl = document.getElementById('dln-history-body');
                if (loadingEl) loadingEl.classList.add('hidden');
                if (bodyEl) bodyEl.classList.remove('hidden');

                // Initialize pagination
                dlnUpdatePaginationInfo();
                dlnRenderPage();

                function dlnUpdatePaginationInfo() {
                    ...
                    document.getElementById('dln-total-records').textContent = totalRecords;
                    document.getElementById('dln-total-pages').textContent = totalPages;
                    document.getElementById('dln-current-page').textContent = paginationState.currentPage;
                    document.getElementById('dln-history-count').textContent = Math.min(
                        paginationState.itemsPerPage, 
                        totalRecords - ((paginationState.currentPage - 1) * paginationState.itemsPerPage)
                    );
                    
                    // Update button states
                    const prevBtn = document.getElementById('dln-prev-page');
                    const nextBtn = document.getElementById('dln-next-page');
                ...
                function dlnRenderPage() {
                    const dlnStartIdx = (paginationState.currentPage - 1) * paginationState.itemsPerPage;
                    const endIdx = Math.min(dlnStartIdx + paginationState.itemsPerPage, paginationState.filteredData.length);
                    const pageData = paginationState.filteredData.slice(dlnStartIdx, endIdx);

                    const tbody = document.getElementById('dln-history-rows');
                    const empty = document.getElementById('dln-history-empty');
                        ...
                        return `
                            <tr>
                                ...
                                <td class="p-2 text-center">
                                    <button class="btn-use-history bg-white border border-amber-200 text-amber-700 px-2 py-1 rounded text-[10px] font-bold hover:bg-amber-50" data-idx="${dlnStartIdx + idx}">USE</button>
                                </td>
                            </tr>
                        `;
                    }).join('');
                    ...
                    tbody.querySelectorAll('.btn-use-history').forEach(btn => {
                        btn.addEventListener('click', (e) => {
                            ...
                            if (!row) return;
                            dlnAddItemToCartWithOverrides(stock, { rate: row.rate, disc: row.disc });
                            document.getElementById('dln-modal-backdrop').classList.add('hidden');
                            dlnCloseCreateStockModal();
                        });
                    });

                    const useLastBtn = document.getElementById('dln-btn-use-last-history');
                ...
                document.getElementById('dln-prev-page').addEventListener('click', () => {
                ...
                document.getElementById('dln-next-page').addEventListener('click', () => {
                ...
                document.getElementById('dln-items-per-page').addEventListener('change', (e) => {
    ...
    // --- MODAL: OTHER CHARGES ---
    function dlnOpenOtherChargesModal() {
        const modal = document.getElementById('dln-other-charges-modal-backdrop');
        const content = document.getElementById('dln-other-charges-modal-content');
        if (!modal || !content) return;
        
        modal.classList.remove('hidden');
        
        // Update the total charges display
        dlnUpdateTotalOtherCharges();
        
        // Render the charges list
        const chargesList = document.getElementById('dln-other-charges-list');
        if (chargesList) {
            chargesList.innerHTML = dlnRenderOtherChargesList();
        }
        
        // Attach event listeners for remove buttons
        dlnAttachOtherChargesListeners();
        
        // Auto-complete functionality for charge name
        const chargeNameInput = document.getElementById('dln-charge-name');
        const suggestionsContainer = document.getElementById('dln-charge-name-suggestions');
        
        // Load existing other charges types for auto-complete
        let existingCharges = [];
        
        async function dlnLoadExistingCharges() {
            try {
                const response = await window.api.get('/inventory/dln/api/other-charges/types');
            ...
            if (filteredCharges.length > 0) {
                ...
                suggestionsContainer.querySelectorAll('.charge-suggestion-item').forEach(item => {
                    item.addEventListener('click', function() {
                        ...
                        if (name) document.getElementById('dln-charge-name').value = name;
                        if (type) document.getElementById('dln-charge-type').value = type;
                        if (hsnSac) document.getElementById('dln-charge-hsn').value = hsnSac;
                        if (gstRate) document.getElementById('dln-charge-gst').value = gstRate;
        ...
        // Add charge button
        document.getElementById('dln-add-charge-btn').onclick = () => {
            const name = document.getElementById('dln-charge-name').value.trim();
            const hsnSac = document.getElementById('dln-charge-hsn').value.trim();
            const amount = parseFloat(document.getElementById('dln-charge-amount').value);
            const gstRate = parseFloat(document.getElementById('dln-charge-gst').value) || 0;
            const type = document.getElementById('dln-charge-type').value;
            ...
            // Clear inputs
            document.getElementById('dln-charge-name').value = '';
            document.getElementById('dln-charge-hsn').value = '';
            document.getElementById('dln-charge-amount').value = '';
            document.getElementById('dln-charge-gst').value = '';
        ...
        // Close button
        document.getElementById('dln-close-other-charges-modal').onclick = () => modal.classList.add('hidden');
        document.getElementById('dln-cancel-other-charges').onclick = () => modal.classList.add('hidden');
        
        // Save button
        document.getElementById('dln-save-other-charges').onclick = () => {
            modal.classList.add('hidden');
        };
    }
    
    function dlnUpdateTotalOtherCharges() {
        const totalElement = document.getElementById('dln-total-other-charges');
        if (totalElement) {
            totalElement.textContent = dlnFormatCurrency(dlnGetTotalOtherCharges());
        }
    }
    
    function dlnAttachOtherChargesListeners() {
        // Attach listeners to remove buttons
        document.querySelectorAll('.btn-remove-charge').forEach(btn => {
            btn.onclick = (e) => {
                const index = parseInt(e.target.dataset.index);
                dlnRemoveOtherCharge(index);
    ...
    function dlnOpenCreateStockModal() {
        const subModal = document.getElementById('dln-sub-modal-backdrop');
        const subContent = document.getElementById('dln-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="dln-close-sub-modal" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            
            <form id="dln-create-stock-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div id="dln-batch-field-container">
                ...
                <input type="hidden" id="dln-stockData" name="stockData" value="">
                <input type="hidden" id="dln-selectedBatchIndex" name="selectedBatchIndex" value="">
                <div>
                ...
                <div class="col-span-2 pt-6 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="dln-cancel-create-stock" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        document.getElementById('dln-close-sub-modal').onclick = dlnCloseCreateStockModal;
        document.getElementById('dln-cancel-create-stock').onclick = dlnCloseCreateStockModal;

        document.getElementById('dln-create-stock-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            ...
            try {
                const res = await window.api.post('/inventory/dln/api/stocks', data);
                ...
                const refreshRes = await window.api.get('/inventory/dln/api/stocks');
    ...
    function dlnOpenEditStockModal(stock) {
        const subModal = document.getElementById('dln-sub-modal-backdrop');
        const subContent = document.getElementById('dln-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            ...
            <form id="dln-edit-stock-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                <div class="col-span-2">
                    <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Item Description *</label>
                ...
                <!-- Hidden fields for batch management -->
                <input type="hidden" id="dln-stockData" name="stockData" value="${JSON.stringify(stock).replace(/"/g, '&quot;')}">
                <input type="hidden" id="dln-selectedBatchIndex" name="selectedBatchIndex" value="">
                <div>
                    <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Part No (P/No)</label>
                ...
                <div class="col-span-2 pt-6 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="dln-cancel-edit-stock" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        document.getElementById('dln-close-sub-modal').onclick = dlnCloseCreateStockModal;
        document.getElementById('dln-cancel-edit-stock').onclick = dlnCloseCreateStockModal;
        ...
        setTimeout(() => {
            const form = document.getElementById('dln-edit-stock-form');
            if (form) {
                ...
                if (stock.batches && Array.isArray(stock.batches) && stock.batches.length > 0) {
                    if (stock.batches.length > 1) {
                        ...
                        dlnShowBatchSelectionForEdit(stock);
                    } else {
        ...
        document.getElementById('dln-edit-stock-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            ...
            const stockData = document.getElementById('dln-stockData') ? document.getElementById('dln-stockData').value : null;
            const selectedBatchIndex = document.getElementById('dln-selectedBatchIndex') ? document.getElementById('dln-selectedBatchIndex').value : null;
    ...
    function dlnCloseCreateStockModal() {
        const el = document.getElementById('dln-sub-modal-backdrop');
        if (el) el.classList.add('hidden');
        
        // Reset the batch field to original state
        dlnResetBatchFieldToOriginal();
    }
    
    // Function to reset batch field to original structure
    function dlnResetBatchFieldToOriginal() {
        ...
        const batchContainer = document.getElementById('dln-batch-field-container');
        if (batchContainer) {
    ...
    function dlnShowBatchSelectionForEdit(stock) {
    ...
    function dlnOpenPartyModal() {
        ...
        content.innerHTML = `
            <div class="p-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <input type="text" id="dln-party-search" placeholder="Search Firm or GSTIN..." class="pl-8 pr-3 py-1.5 border border-gray-300 rounded text-xs w-48 focus:ring-1 focus:ring-blue-500 outline-none">
                    ...
                    <button id="dln-btn-create-party" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-medium shadow flex items-center gap-1 transition-colors">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        New Party
                    </button>
                    
                    <button id="dln-close-party-modal" class="text-gray-400 hover:text-gray-800 text-2xl leading-none ml-2">&times;</button>
                </div>
            </div>
            
            <div class="p-4 grid gap-3 max-h-[60vh] overflow-y-auto bg-gray-50" id="dln-party-list-container">
                </div>
        `;

        // 2. Helper to render the list rows
        const dlnRenderPartyList = (data) => {
            const container = document.getElementById('dln-party-list-container');
            if (!container) return;
            ...
            container.innerHTML = data.map(party => `
                <div class="party-item border border-gray-200 p-3 rounded-lg hover:border-blue-400 hover:shadow-md cursor-pointer flex justify-between items-center transition-all bg-white group" data-id="dln-${party.id}">
                    <div>
            ...
            container.querySelectorAll('.party-item').forEach(div => {
                div.addEventListener('click', () => {
                    const id = parseInt(div.getAttribute('data-id').replace('dln-', ''));
        ...
        // Initial Render
        dlnRenderPartyList(state.parties);

        // 3. Attach Event Listeners
        document.getElementById('dln-close-party-modal').addEventListener('click', () => {
            modal.classList.add('hidden');
        });

        document.getElementById('dln-btn-create-party').addEventListener('click', dlnOpenCreatePartyModal); // <--- OPEN SUB MODAL

        // Search Logic
        const searchInput = document.getElementById('dln-party-search');
        if (searchInput) {
            searchInput.focus();
            searchInput.addEventListener('input', (e) => {
                const term = e.target.value.toLowerCase();
                ...
                dlnRenderPartyList(filtered);
            });
        }
    }

    function dlnOpenCreatePartyModal() {
        const subModal = document.getElementById('dln-sub-modal-backdrop');
        const subContent = document.getElementById('dln-sub-modal-content');
        if (!subModal || !subContent) return;
        ...
        subContent.innerHTML = `
            <div class="bg-slate-800 p-4 flex justify-between items-center text-white">
                ...
                <button id="dln-close-sub-modal-party" class="hover:text-red-300 text-lg transition-colors">&times;</button>
            </div>
            
            <form id="dln-create-party-form" class="p-6 grid grid-cols-2 gap-x-6 gap-y-4">
                
                <div class="col-span-2">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Firm Name *</label>
                    <input type="text" name="firm" id="dln-new-party-firm" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none placeholder-gray-300" placeholder="e.g. M/S Global Enterprises">
                </div>

                <div class="col-span-2 md:col-span-1">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">GSTIN</label>
                    <div class="flex gap-2">
                        <input type="text" name="gstin" id="dln-new-party-gstin" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none font-mono uppercase" placeholder="27ABCDE1234F1Z5" maxlength="15">
                        <button type="button" id="dln-btn-fetch-gst" class="bg-orange-500 hover:bg-orange-600 text-white px-3 rounded text-xs font-bold shadow transition-colors flex items-center justify-center min-w-[60px]">
                            FETCH
                        </button>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="state" id="dln-new-party-state" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">State Code</label>
                    <input type="number" name="state_code" id="dln-new-party-state-code" class="w-full border border-gray-300 rounded px-3 py-2 text-sm bg-gray-50 outline-none" readonly>
                </div>

                <div class="col-span-2">
                    <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Address</label>
                    <textarea name="addr" id="dln-new-party-addr" rows="2" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none resize-none"></textarea>
                </div>
                
                <div class="grid grid-cols-2 gap-4 col-span-2">
                    <div>
                         <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">Pincode</label>
                         <input type="number" name="pin" id="dln-new-party-pin" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none">
                    </div>
                    <div>
                         <label class="block text-[10px] font-bold text-gray-500 mb-1 uppercase">PAN</label>
                         <input type="text" name="pan" id="dln-new-party-pan" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none uppercase font-mono" maxlength="10">
                    </div>
                </div>

                <div class="col-span-2 pt-4 border-t border-gray-100 flex justify-end gap-3 mt-2">
                    <button type="button" id="dln-cancel-create-party" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium hover:bg-gray-100 rounded">Cancel</button>
        ...
        // --- LISTENERS ---
        const dlnCloseFunc = () => document.getElementById('dln-sub-modal-backdrop').classList.add('hidden');
        document.getElementById('dln-close-sub-modal-party').addEventListener('click', dlnCloseFunc);
        document.getElementById('dln-cancel-create-party').addEventListener('click', dlnCloseFunc);

        // Auto-detect State Code from GSTIN (Input Event)
        const gstinInput = document.getElementById('dln-new-party-gstin');
        gstinInput.addEventListener('input', (e) => {
            const val = e.target.value.toUpperCase();
            e.target.value = val;
            if (val.length >= 2 && !isNaN(val.substring(0, 2))) {
                document.getElementById('dln-new-party-state-code').value = val.substring(0, 2);
            }
            // Auto extract PAN
            if (val.length >= 12) {
                document.getElementById('dln-new-party-pan').value = val.substring(2, 12);
            }
        });

        // [NEW] API Fetch Listener
        document.getElementById('dln-btn-fetch-gst').addEventListener('click', function () {
            dlnFetchPartyByGST(this); // Call the helper function
        });

        // Form Submit
        document.getElementById('dln-create-party-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            ...
            try {
                const res = await window.api.post('/inventory/dln/api/parties', data);
                const result = await res.json();
                ...
                // Refresh Parties
                const refreshRes = await window.api.get('/inventory/dln/api/parties');
    ...
    // --- CART LOGIC ---
    function dlnAddItemToCart(stockItem) {
    ...
    function dlnRefreshTable() {
        document.getElementById('dln-items-container').innerHTML = dlnRenderItemsList();
        document.getElementById('dln-totals-section').innerHTML = dlnRenderTotals();
        dlnAttachTableListeners();
    }

    // --- EVENT ATTACHMENTS (ROBUST) ---
    function dlnAttachGlobalListeners() {
        const addBtn = document.getElementById('dln-btn-add-item');
        if (addBtn) addBtn.onclick = dlnOpenStockModal;

        const partyBtn = document.getElementById('dln-btn-select-party');
        if (partyBtn) partyBtn.onclick = dlnOpenPartyModal;

        // Note: The change party button event listener is handled in dlnRenderPartyCard() after async rendering
        ...
        const consigneePinInput = document.getElementById('dln-consignee-pin');
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'F2') {
                ...
                e.preventDefault();
                dlnOpenOtherChargesModal();
            }
        });

        const typeSelector = document.getElementById('dln-billTypeSelector');
        if (typeSelector) {
        ...
        const referenceNoInput = document.getElementById('dln-reference-no');
        if (referenceNoInput) {
        ...
        // Vehicle No input
        const vehicleNoInput = document.getElementById('dln-vehicle-no');
        if (vehicleNoInput) {
        ...
        const dispatchThroughInput = document.getElementById('dln-dispatch-through');
        ...
        // Narration textarea
        const narrationInput = document.getElementById('dln-narration');
        if (narrationInput) {
        ...
        const resetBtn = document.getElementById('dln-btn-reset');
        if (resetBtn) {
            resetBtn.onclick = () => {
        ...
        const saveBtn = document.getElementById('dln-btn-save');
        if (saveBtn) {
            saveBtn.onclick = async () => {
                ...
                try {
                    ...
                    const response = await window.api.post('/inventory/dln/api/bills', billData);
                    ...
                    const invoiceData = dlnGenerateInvoiceData(billData, result.billId);
                    
                    // Add GST status to invoice data
                    invoiceData.gstEnabled = gstEnabled;
                    
                    // Export to PDF
                    dlnExportBillToPdf({id: result.billId, bno: invoiceData.meta.dlnBillNo});
    ...
    function dlnAttachTableListeners() {
        // Inputs
        document.querySelectorAll('.tbl-input').forEach(input => {
            input.oninput = (e) => {
                ...
                if (totalDiv) totalDiv.innerText = dlnFormatCurrency(rowTotal);
    ...
    async function dlnFetchPartyByGST(buttonElement) {
        const gstinInput = document.getElementById('dln-new-party-gstin');
        ...
        try {
            ...
            dlnPopulatePartyFromRapidAPI(partyData, gstin);
    ...
    function dlnPopulatePartyFromRapidAPI(partyData, gstin) {
        ...
        const address = dlnFormatPowerfulGSTINAddress(partyData) || '';
        const pinCode = dlnExtractPowerfulGSTINPinCode(partyData) || '';
        ...
        // Firm Name
        if(document.getElementById('dln-new-party-firm')) 
            document.getElementById('dln-new-party-firm').value = displayName;

        // Address
        if(document.getElementById('dln-new-party-addr')) 
            document.getElementById('dln-new-party-addr').value = address;

        // State
        if(document.getElementById('dln-new-party-state')) 
            document.getElementById('dln-new-party-state').value = stateName;

        // Pincode
        if(document.getElementById('dln-new-party-pin')) 
            document.getElementById('dln-new-party-pin').value = pinCode;

        // State Code (Auto-fill from GSTIN first 2 chars)
        if(gstin && gstin.length >= 2) {
             const scInput = document.getElementById('dln-new-party-state-code');
             if(scInput) scInput.value = gstin.substring(0, 2);
        }

        // PAN (Auto-fill from GSTIN chars 3-12)
        if(gstin && gstin.length >= 12) {
            const panInput = document.getElementById('dln-new-party-pan');
            if(panInput) panInput.value = gstin.substring(2, 12);
        }
    }

    function dlnFormatPowerfulGSTINAddress(partyData) {
    ...
    function dlnExtractPowerfulGSTINPinCode(partyData) {
        if (!partyData || !partyData.place_of_business_principal) return '';
        ...
        // Validate PIN dlnFormat (6 digits)
        if (/^\d{6}$/.test(pinStr)) {
            return pinStr;
        }
    ...
    function dlnGenerateInvoiceData(billData, billId) {
        ...
        if (state.otherCharges) {
            state.otherCharges.forEach(charge => {
                ...
                const chargeGstRate = parseFloat(charge.gstRate) || 0;
                // Calculate GST amount only if GST is enabled
                const dlnChargeGstAmount = state.gstEnabled !== false ? (chargeAmount * chargeGstRate) / 100 : 0;
                tempOtherChargesTotal += chargeAmount;
                tempOtherChargesGstTotal += dlnChargeGstAmount;
            });
        }
        ...
        const processedOtherCharges = state.otherCharges.map(charge => {
            ...
            return {
                ...charge,
                gstAmount: dlnChargeGstAmount,
                totalAmount: chargeAmount + dlnChargeGstAmount
            };
        });
   ...
   function dlnExportInvoiceToExcel(invoiceData) {
    ...
    const dlnCreateCell = (v, s) => ({ v: v || "", s: s || {} });
    const ws_data = [];
    ...
    ws_data.push([dlnCreateCell(excelInvoiceTitle, styles.title)]);
    ws_data.push([]); // Spacer

    // --- DETAILS SECTION ---
    ws_data.push([
        dlnCreateCell("SELLER:", { font: { bold: true } }), "", "", "", "", "", 
        dlnCreateCell("Invoice No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
        dlnCreateCell(invoiceData.meta.dlnBillNo, { alignment: { horizontal: "left" } })
    ]);
    
    // Use the actual firm name instead of hardcoded "Your Company Name"
    ws_data.push([
        dlnCreateCell(invoiceData.sellerName || "Your Company Name", { font: { bold: true } }), "", "", "", "", "", 
        dlnCreateCell("Date:", { font: { bold: true }, alignment: { horizontal: "right" } }),
        dlnCreateCell(invoiceData.meta.billDate, { alignment: { horizontal: "left" } })
    ]);
    
    // PO No and Vehicle No in sequence
    if (invoiceData.meta.referenceNo) {
        ws_data.push([
            dlnCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            dlnCreateCell("PO No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            dlnCreateCell(invoiceData.meta.referenceNo, { alignment: { horizontal: "left" } })
        ]);
    }
    ...
    if (invoiceData.meta.vehicleNo) {
        ws_data.push([
            dlnCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            dlnCreateCell("Vehicle No:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            dlnCreateCell(invoiceData.meta.vehicleNo, { alignment: { horizontal: "left" } })
        ]);
    }
    ...
    if (invoiceData.meta.dispatchThrough) {
        ws_data.push([
            dlnCreateCell("", { font: { bold: true } }), "", "", "", "", "", 
            dlnCreateCell("Dispatched Through:", { font: { bold: true }, alignment: { horizontal: "right" } }),
            dlnCreateCell(invoiceData.meta.dispatchThrough, { alignment: { horizontal: "left" } })
        ]);
    }
    
    ws_data.push([dlnCreateCell("BUYER (BILL TO):", { font: { bold: true } })]);
    ...
    ws_data.push(headers.map(h => dlnCreateCell(h, styles.header)));

    // --- TABLE ITEMS ---
    invoiceData.cart.forEach((item, index) => {
        const lineTotal = item.qty * item.rate * (1 - (item.disc || 0) / 100);
        ws_data.push([
            dlnCreateCell(index + 1, styles.cellCenter),
            dlnCreateCell(item.item, styles.cellLeft),
            dlnCreateCell(item.hsn, styles.cellCenter),
            dlnCreateCell(item.qty, styles.cellCenter),
            dlnCreateCell(item.uom, styles.cellCenter),
            dlnCreateCell(item.rate, styles.cellRight),
            dlnCreateCell(item.disc || 0, styles.cellRight),
            dlnCreateCell(item.grate, styles.cellRight),
            dlnCreateCell(lineTotal.toFixed(2), styles.cellRight)
        ]);
        
        // Add narration row if it exists
        if (item.narration) {
            ws_data.push([
                dlnCreateCell('', styles.cellCenter),
                dlnCreateCell('Narration: ' + item.narration, styles.cellLeft),
                dlnCreateCell('', styles.cellCenter),
                dlnCreateCell('', styles.cellCenter),
                dlnCreateCell('', styles.cellCenter),
                dlnCreateCell('', styles.cellRight),
                dlnCreateCell('', styles.cellRight),
                dlnCreateCell('', styles.cellRight),
                dlnCreateCell('', styles.cellRight)
            ]);
    ...
    if (invoiceData.otherCharges && invoiceData.otherCharges.length > 0) {
        ...
        invoiceData.otherCharges.forEach(charge => {
            ...
            ws_data.push([
                dlnCreateCell("", styles.cellCenter),
                dlnCreateCell(`${charge.type} (${charge.name})`, styles.cellLeft),
                dlnCreateCell(charge.hsnSac || "", styles.cellCenter), // HSN/SAC
                dlnCreateCell("", styles.cellCenter),
                dlnCreateCell("", styles.cellCenter),
                dlnCreateCell("", styles.cellRight),
                dlnCreateCell("", styles.cellRight),
                dlnCreateCell(charge.gstRate || 0, styles.cellRight), // GST rate
                dlnCreateCell(dlnFormatCurrency(charge.amount), styles.cellRight)
            ]);
            ...
            if (charge.gstAmount > 0) {
                ws_data.push([
                    dlnCreateCell("", styles.cellCenter),
                    dlnCreateCell(`GST on ${charge.type} (${charge.name})`, styles.cellLeft),
                    dlnCreateCell(charge.hsnSac || "", styles.cellCenter), // HSN/SAC
                    dlnCreateCell("", styles.cellCenter),
                    dlnCreateCell("", styles.cellCenter),
                    dlnCreateCell("", styles.cellRight),
                    dlnCreateCell("", styles.cellRight),
                    dlnCreateCell(0, styles.cellRight), // No GST on GST
                    dlnCreateCell(dlnFormatCurrency(charge.gstAmount), styles.cellRight)
                ]);
            }
        });
    }
    ...
    for (let i = 0; i < (minRows - invoiceData.cart.length - (invoiceData.otherCharges ? invoiceData.otherCharges.length : 0)); i++) {
        ...
        const emptyRow = Array.from({length: 9}, () => dlnCreateCell('', styles.cellCenter));
        ws_data.push(emptyRow);
    }
    
    // --- FOOTER SECTION ---
            
    // Calculate the same totals as in dlnRenderTotals for consistency
    let totalTaxable = 0;
    let totalTaxAmount = 0;
    ...
    if (invoiceData.otherCharges) {
        invoiceData.otherCharges.forEach(charge => {
            const chargeAmount = parseFloat(charge.amount) || 0;
            const chargeGstRate = parseFloat(charge.gstRate) || 0;
            const dlnChargeGstAmount = gstEnabled ? (chargeAmount * chargeGstRate) / 100 : 0;
            otherChargesSubtotal += chargeAmount;
            otherChargesGstTotal += dlnChargeGstAmount;
        });
    }
    ...
    const dlnAddFooterRow = (label, val, isWordsRow = false) => {
        // Fix: Create unique empty cells for this row
        const row = Array.from({length: 9}, () => dlnCreateCell("", {}));
        ...
        if (isWordsRow) {
            ...
            row[0] = dlnCreateCell("Amount in Words:\n" + dlnNumToIndianRupees(roundedTotal || 0), styles.words);
        }

        // Totals at columns 6, 7 and 8 - merge G and H columns
        row[6] = dlnCreateCell(label, styles.totalLabel); // Column G will contain the label
        row[7] = dlnCreateCell("", styles.totalLabel); // Column H will be merged with G
        row[8] = dlnCreateCell((typeof val === 'number' ? val : 0).toFixed(2), styles.totalValue);
    ...
    ws_data.push(dlnAddFooterRow("Taxable Value", totalTaxableValue, true));

    // 2. Taxes
    if (invoiceData.meta.billType === 'intra-state') {
        ws_data.push(dlnAddFooterRow("CGST", finalCgstAmount));
        ws_data.push(dlnAddFooterRow("SGST", finalSgstAmount));
    } else {
        ws_data.push(dlnAddFooterRow("IGST", finalIgstAmount));
    }
    ...
    if (invoiceData.otherCharges && invoiceData.otherCharges.length > 0) {
        ...
        invoiceData.otherCharges.forEach(charge => {
            ws_data.push(dlnAddFooterRow(`${charge.type} (${charge.name})`, charge.amount));
            
            // Add GST on the charge if applicable
            if (charge.gstAmount > 0) {
                ws_data.push(dlnAddFooterRow(`GST on ${charge.type} (${charge.name})`, charge.gstAmount));
            }
        });
    }
    ...
    // Add Round Off row
    ws_data.push(dlnAddFooterRow("Round Off", roundOff));

    const rFinal = Array.from({length: 9}, () => dlnCreateCell("", {}));
    rFinal[6] = dlnCreateCell("GRAND TOTAL", styles.header);
    rFinal[7] = dlnCreateCell("", styles.header);
    rFinal[8] = dlnCreateCell(Math.round(excelGrandTotal).toFixed(2), styles.header);
    ws_data.push(rFinal);
    ...
    const hsnHeaderRow = Array.from({length: 9}, () => dlnCreateCell("", styles.cellCenter));
    hsnHeaderRow[0] = dlnCreateCell("HSN Summary", styles.header);
    hsnHeaderRow[1] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[2] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[3] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[4] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[5] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[6] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[7] = dlnCreateCell("", styles.cellCenter);
    hsnHeaderRow[8] = dlnCreateCell("", styles.cellCenter);
    ws_data.push(hsnHeaderRow);
    
    // HSN Summary table headers
    const hsnHeadersRow = Array.from({length: 9}, () => dlnCreateCell("", styles.cellCenter));
    hsnHeadersRow[0] = dlnCreateCell("HSN", styles.header);
    hsnHeadersRow[1] = dlnCreateCell("", styles.cellCenter);
    hsnHeadersRow[2] = dlnCreateCell("Taxable Value", styles.header);
    hsnHeadersRow[3] = dlnCreateCell("", styles.cellCenter);
    hsnHeadersRow[4] = dlnCreateCell("IGST Amount", styles.header);
    hsnHeadersRow[5] = dlnCreateCell("CGST Amount", styles.header);
    hsnHeadersRow[6] = dlnCreateCell("SGST Amount", styles.header);
    hsnHeadersRow[7] = dlnCreateCell("Total Tax", styles.header);
    hsnHeadersRow[8] = dlnCreateCell("", styles.cellCenter);
    ws_data.push(hsnHeadersRow);
    
    // Add HSN Summary rows
    Object.values(hsnSummary).forEach(hsnData => {
        const hsnRow = Array.from({length: 9}, () => dlnCreateCell("", styles.cellCenter));
        hsnRow[0] = dlnCreateCell(hsnData.hsn, styles.cellLeft); // Left-aligned HSN code
        hsnRow[1] = dlnCreateCell("", styles.cellCenter);
        hsnRow[2] = dlnCreateCell(hsnData.taxableValue.toFixed(2), styles.cellRight);
        hsnRow[3] = dlnCreateCell("", styles.cellCenter);
        hsnRow[4] = dlnCreateCell(hsnData.igstAmount.toFixed(2), styles.cellRight);
        hsnRow[5] = dlnCreateCell(hsnData.cgstAmount.toFixed(2), styles.cellRight);
        hsnRow[6] = dlnCreateCell(hsnData.sgstAmount.toFixed(2), styles.cellRight);
        hsnRow[7] = dlnCreateCell((hsnData.igstAmount + hsnData.cgstAmount + hsnData.sgstAmount).toFixed(2), styles.cellRight);
        hsnRow[8] = dlnCreateCell("", styles.cellCenter);
        ws_data.push(hsnRow);
    });
    ...
    if (invoiceData.meta.narration) {
        ws_data.push([]); // Empty row for spacing
        const narrationRow = Array.from({length: 9}, () => dlnCreateCell("", styles.cellCenter));
        narrationRow[0] = dlnCreateCell("Narration: " + (invoiceData.meta.narration || ""), { font: { bold: true }, alignment: { horizontal: "left", vertical: "top", wrapText: true } });
        // Span the narration across all 9 columns
        for (let i = 1; i < 9; i++) {
            narrationRow[i] = dlnCreateCell("", styles.cellCenter);
        }
    ...
    XLSX.writeFile(wb, `Invoice_${invoiceData.meta.dlnBillNo}.xlsx`);
}

// Export to PDF
    async function dlnExportBillToPdf(bill) {
        try {
            if (!bill || !bill.id) {
            ...
            const dlnContentType = (response.headers.get('content-type') || '').toLowerCase();
            if (!dlnContentType.includes('application/pdf')) {
                const text = await response.text().catch(() => '');
                throw new Error(`Expected a PDF but received: ${dlnContentType || 'unknown content-type'}.
        Server response: ${text ? text.slice(0, 200) : ''}`);
            ...
            a.href = url;
            const dlnBillNo = (bill.bno || `BILL-${bill.id}`).toString().replace(/[^a-zA-Z0-9._-]/g, '_');
            a.download = `Invoice_${dlnBillNo}.pdf`;
...
function dlnNumToIndianRupees(num) {
    if (!num) return "";
    ...
    const dlnConvert = (n) => {
        if (n < 100) return dlnFormat(n);
        if (n < 1000) return a[Math.floor(n / 100)] + "Hundred " + (n % 100 ? "and " + dlnConvert(n % 100) : "");
        if (n < 100000) return dlnConvert(Math.floor(n / 1000)) + "Thousand " + (n % 1000 ? dlnConvert(n % 1000) : "");
        if (n < 10000000) return dlnConvert(Math.floor(n / 100000)) + "Lakh " + (n % 100000 ? dlnConvert(n % 100000) : "");
        return dlnConvert(Math.floor(n / 10000000)) + "Crore " + (n % 10000000 ? dlnConvert(n % 10000000) : "");
    };

    const parts = num.toString().split(".");
    const rupees = dlnConvert(parseInt(parts[0]));
    const paise = parts[1] ? dlnConvert(parseInt(parts[1].substring(0, 2).padEnd(2, '0'))) : "";
